...
function buildLevel(spec: Spec, index: number): LevelData {
  ...
  if (spec.act >= 2 && w > 120 && !spec.boss) {
    ...
    for (let seg = 0; seg < 3; seg++) {
      ...
      if (seg < 2) { const gap = 4 + Math.floor(r() * 2); bridges.push({ x: (px + platW) * TILE - 12, y: H * TILE, w: gap * TILE + 24 }); px += platW + gap; }
    }
  }
  ...
  const spanClear = (b: { x: number; y: number; w: number }) => {
    const ty = Math.round(b.y / TILE);
    const x0 = Math.ceil((b.x + 14) / TILE), x1 = Math.floor((b.x + b.w - 14) / TILE);
    for (let cx = x0; cx < x1; cx++) for (let cy = ty - 2; cy <= ty; cy++) {
