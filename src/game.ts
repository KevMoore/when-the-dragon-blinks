...
export class Game {
  ...
  spawners: { kind: EntityKind; x: number; y: number; every: number; max: number; t: number }[] = [];   // designer wave points
  ...
  private smoothTerrain(level: LevelData) {
    const h = level.height, w = level.width;
    const isT = (ch: string) => ch === '#' || ch === 'g';
    const surf = (x: number) => { for (let y = 0; y < h; y++) if (isT(level.tiles[y]?.[x] || '.')) return y; return -1; };
    const setT = (x: number, y: number, ch: string) => { const r = level.tiles[y]; level.tiles[y] = r.slice(0, x) + ch + r.slice(x + 1); };
    for (let pass = 0; pass < 2; pass++) {
      for (let x = 1; x < w - 1; x++) {
        const s = surf(x), l = surf(x - 1), r = surf(x + 1);
        if (s < 0 || l < 0 || r < 0) continue;
        if (s > l && s > r) {                                    // 1-wide crack → fill to the lower neighbour
          for (let y = Math.max(l, r); y < s; y++) setT(x, y, '#');
        } else if (s < l && s < r) {                             // 1-wide spike → shave to the higher neighbour
          for (let y = s; y < Math.min(l, r); y++) setT(x, y, '.');
        }
      }
    }
    // re-cap: terrain with air above becomes grass, the rest stone
    for (let x = 0; x < w; x++) for (let y = 0; y < h; y++) {
      if (!isT(level.tiles[y]?.[x] || '.')) continue;
  ...
  private updateGems(dt: number) {
    ...
    for (const g of this.gems) {
      if (g.taken) { if (g.rt > 0 && (g.rt -= dt) <= 0) { g.taken = false; g.x = (g as any).hx ?? g.x; g.y = (g as any).hy ?? g.y; } continue; }
      if ((g as any).hx === undefined) { (g as any).hx = g.x; (g as any).hy = g.y; }
      // magnet: nearby gems drift eagerly to the bearer of the eye-shard
      const dx = pcx - (g.x + 12), dy = pcy - (g.y + 12), d = Math.hypot(dx, dy);
      if (d < 110 && d > 1) { const pull = (1 - d / 110) * 420 * dt; g.x += dx / d * pull; g.y += dy / d * pull; }
  ...
  solidsForRect(r: Rect, world: WorldState = this.world): Solid[] {
    ...
    for (const b of this.bridges) {
      if (b.x + b.w < r.x - TILE || b.x > r.x + r.w + TILE) continue;
      const by = b.y + b.sag;
      if (by + 12 < r.y - TILE || by > r.y + r.h + TILE) continue;
  ...
  onOneWayGround(e: { x: number; y: number; w: number; h: number }) {
    ...
    const feet = e.y + e.h;
    for (const b of this.bridges) if (e.x + e.w > b.x && e.x < b.x + b.w && Math.abs(feet - (b.y + b.sag)) < 6) return true;
    return false;
  }
  ...
  private updatePlaying(dt: number) {
    ...
    this.spawners.forEach((sp, si) => {
      ...
      if (sp.t > 0) return;
      sp.t = sp.every;
      const alive = this.enemies.reduce((n, e) => n + ((e as any)._sp === si && e.alive ? 1 : 0), 0);
      const onScreen = sp.x > this.camera.x - 200 && sp.x < this.camera.x + LOGICAL_W + 200;
      if (alive < sp.max && onScreen) {
        const e = new Enemy(sp.kind, sp.x, sp.y); (e as any)._sp = si;
        this.enemies.push(e); this.snapEnemySpawn(e);
  ...
  private drawRemnants(c: CanvasRenderingContext2D) {
    for (const r of this.remnants) {
      ...
      const x = r.x - this.camera.x, y = r.y - this.camera.y;
      c.save(); c.globalCompositeOperation = 'lighter'; c.globalAlpha = (1 - p) * 0.75;
      if (sheet && sheet.ready) {
        c.translate(x, y); c.scale(r.face * (1 - p * 0.25), 1 + p * 0.55);
        c.shadowColor = r.night ? '#a9d6ff' : '#ffd777'; c.shadowBlur = 22;
        sheet.blit(c, 0, r.h, false);
      } else {
        const g = c.createRadialGradient(x, y, 0, x, y, r.h * 0.5);
        g.addColorStop(0, r.night ? 'rgba(169,214,255,.8)' : 'rgba(255,215,119,.8)'); g.addColorStop(1, 'rgba(0,0,0,0)');
        c.fillStyle = g; c.beginPath(); c.arc(x, y, r.h * 0.5, 0, Math.PI * 2); c.fill();
      }
      c.restore(); c.globalAlpha = 1;
    }
  }
  ...
  private tintedBridge(): HTMLCanvasElement | null {
    ...
    const tints: Record<string, string> = { mountain: 'rgba(150,80,50,.16)', bridge: 'rgba(50,110,110,.3)', cavern: 'rgba(110,60,150,.34)', sunless: 'rgba(55,75,115,.4)', arena: 'rgba(140,45,35,.3)' };
    g.globalCompositeOperation = 'source-atop';
    g.fillStyle = tints[this.level.theme] ?? tints.mountain; g.fillRect(0, 0, cv.width, cv.height);
    if (this.world === 'night') { g.fillStyle = 'rgba(30,45,85,.38)'; g.fillRect(0, 0, cv.width, cv.height); }
