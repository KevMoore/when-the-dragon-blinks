...
try { Object.defineProperty(ctx, 'shadowBlur', { get: () => 0, set: () => { /* disabled */ } }); } catch { /* stubbed ctx */ }
...
function tuneQuality(dt: number, now: number): boolean {
  frameCount++;
  if (dt > 0.021) slowFrames++;             // missed the 60fps budget
  if (dt < 0.0175) fastFrames++;
  if (frameCount < 90) return false;        // judge over ~1.5s windows
  ...
  if (slow > 0.2 && dprScale > 0.5 && now - lastQualityChange > 5000) {
    ...
    lastQualityChange = now; resize(); return true;
  }
  if (fastWindows >= 3 && dprScale < 1 && now - lastQualityChange > 30000) {
    ...
    lastQualityChange = now; fastWindows = 0; resize(); return true;
  }
  return false;
}
