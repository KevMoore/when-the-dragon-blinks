...
export class Input {
  ...
  private fjDown = false; private fjPressTime = 0; private fjJumped = false;
  ...
  constructor(private canvas: HTMLCanvasElement) {
    ...
    controls?.querySelectorAll('button[data-action]').forEach(btn => {
      ...
      if (action === 'firejump') {
        const fd = (e: Event) => { e.preventDefault(); this.fjDown = true; this.fjPressTime = performance.now(); this.fjJumped = false; this.touchPressed.add('attack'); btn.classList.add('active'); };
        const fu = (e: Event) => { e.preventDefault(); this.fjDown = false; btn.classList.remove('active'); };
  ...
  down(action: string): boolean {
    ...
    if (action === 'jump') return k.has(' ') || this.touch.has('jump') || !!this.gpButtons[0] || (this.fjDown && performance.now() - this.fjPressTime >= 140);
  ...
  just(action: string): boolean {
    ...
    if (action === 'jump') {
      ...
      if (this.fjDown && !this.fjJumped && performance.now() - this.fjPressTime >= 140) { this.fjJumped = true; j = true; }
      return j;
    }
