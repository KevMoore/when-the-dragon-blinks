...
type BossState = 'intro' | 'idle' | 'telegraph' | 'attack' | 'recover';
...
export class LanternEater {
  ...
  update(game: Game, dt: number) {
    ...
    switch (this.state) {
      case 'intro': if (this.stateT <= 0) this.enter('idle', 0.6); break;
      case 'idle': if (this.stateT <= 0) this.beginTelegraph(game); break;
      case 'telegraph':
        if (this.stateT <= 0) { this.doAttack(game); this.enter('attack', 0.5); }
        break;
    ...
    if (overlap(this.bodyRect(), game.player.rect())) game.player.hurt(game);
  }

  private enter(s: BossState, t: number) { this.state = s; this.stateT = t; }

  private beginTelegraph(game: Game) {
    // choose an attack for this phase
    const pool: Attack[] = this.phase === 1 ? ['barrage', 'slam']
    ...
    if (this.phase >= 2 && Math.random() < 0.4) game.tryToggleWorld(true);
  }

  private doAttack(game: Game) {
    const p = game.player;
    ...
    if (this.attack === 'barrage') {
      ...
      game.camera.addTrauma(0.8); game.addHitstop(0.05);
  ...
  draw(game: Game, c: CanvasRenderingContext2D) {
    ...
    if (sheet && sheet.ready) {
      ...
      if (this.vulnerable) {
        ...
        g.addColorStop(0, 'rgba(140,210,255,.8)'); g.addColorStop(1, 'rgba(0,0,0,0)');
        c.fillStyle = g; c.beginPath(); c.arc(sx + this.w / 2, sy + 44, 60, 0, Math.PI * 2); c.fill(); c.restore();
      }
      c.globalAlpha = 1;
      if (telegraphNow) {
        c.save(); c.globalAlpha = 0.6 + Math.sin(this.time * 24) * 0.4;
        c.fillStyle = '#ffcf7a'; c.font = 'bold 40px Georgia'; c.textAlign = 'center';
        c.fillText('!', sx + this.w / 2, sy - 14); c.restore();
      }
      return;
    }
    ...
    body.addColorStop(0, this.vulnerable ? '#355472' : '#4a171c'); body.addColorStop(1, '#08050d');
    c.fillStyle = body;
    c.beginPath();
    c.moveTo(-48, -50); c.bezierCurveTo(-90, -10, -68, 80, 0, 80); c.bezierCurveTo(72, 80, 90, -22, 48, -50); c.bezierCurveTo(26, -76, -24, -76, -48, -50); c.fill();
    c.shadowBlur = 0;
    // lantern mask; two halves slide apart as the eye opens
    const open = this.maskOpen * 20;
    c.fillStyle = this.vulnerable ? '#e7f8ff' : '#f0b752';
    c.beginPath(); c.roundRect(-38 - open, -44, 40, 58, 12); c.fill();
    c.beginPath(); c.roundRect(-2 + open, -44, 40, 58, 12); c.fill();
    // exposed eye
    if (this.maskOpen > 0.15) {
      c.save(); c.globalAlpha = this.maskOpen;
      c.shadowColor = this.vulnerable ? '#8bd2ff' : '#d94a3a'; c.shadowBlur = 22;
      c.fillStyle = '#150910'; c.beginPath(); c.ellipse(0, -14, 20, 22, 0, 0, Math.PI * 2); c.fill();
      c.fillStyle = this.vulnerable ? '#8bd2ff' : '#d94a3a';
      c.beginPath(); c.arc(0, -14, this.vulnerable ? 11 : 7, 0, Math.PI * 2); c.fill();
      c.restore();
    }
    // smoky arms
    c.strokeStyle = 'rgba(20,8,18,.82)'; c.lineWidth = 15; c.lineCap = 'round';
    const sway = Math.sin(this.time * 2) * 8;
    c.beginPath(); c.moveTo(-44, 12); c.bezierCurveTo(-94, 22 + sway, -96, 66, -60, 80);
    c.moveTo(44, 12); c.bezierCurveTo(94, 22 - sway, 96, 66, 60, 80); c.stroke();
