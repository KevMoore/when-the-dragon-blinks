...
export class Player {
  ...
  attackTimer = 0;              // shoot animation / muzzle timer
  lastShot: 'bolt' | 'blast' = 'bolt';   // picks the attack vs fire sheet
  shootCd = 0; charging = false; chargeT = 0;
  // juice / animation
  scaleX = 1; scaleY = 1;
  animTime = 0;
  ...
  dragonFireCd = 0;
  dragonBank = 0;            // climb/dive tilt for smooth banking
  crouching = false;
  ...
  private aim(game: Game): [number, number] {
    const i = game.input;
    let dx = 0, dy = 0;
    if (Math.hypot(i.stickX, i.stickY) > 0.3) { dx = i.stickX; dy = i.stickY; }   // analog stick angle
    else {
      if (i.down('left')) dx -= 1; if (i.down('right')) dx += 1;
      if (i.down('up')) dy -= 1; if (i.down('down')) dy += 1;
    }
    if (this.grounded && dy > 0) dy = 0;                 // can't fire into the floor you stand on
  ...
  update(game: Game, dt: number) {
    ...
    if (input.just('attack') && this.shootCd <= 0) { this.fireBolt(game); this.charging = true; this.chargeT = 0; }
    ...
    if (game.save.settings.autoFire && !atkDown && !this.charging && this.shootCd <= 0 && this.dashTime <= 0) this.fireBolt(game);
    if (this.charging) {
      if (atkDown) {
        this.chargeT += dt;
        ...
        if (this.chargeT >= 0.5) this.fireBlast(game);
  ...
  spriteCenterY(game: Game): number {
    if (this.crouching) return this.y + this.h * 0.45;
    return this.y + this.h - (72 * game.spriteScale) / 2;
  }

  private fireBolt(game: Game) {
    ...
    this.shootCd = 0.16; this.attackTimer = 0.14; this.lastShot = 'bolt';
  ...
  private fireBlast(game: Game) {
    ...
    this.attackTimer = 0.24; this.lastShot = 'blast';
    if (dx !== 0) this.facing = dx < 0 ? -1 : 1;
  ...
  private updateDragon(game: Game, dt: number) {
    ...
    let ax = 0, ay = 0;
    if (Math.hypot(i.stickX, i.stickY) > 0.12) { ax = clamp(i.stickX, -1, 1); ay = clamp(i.stickY, -1, 1); }
    else { ax = (i.down('right') ? 1 : 0) - (i.down('left') ? 1 : 0); ay = (i.down('down') ? 1 : 0) - (i.down('up') ? 1 : 0); }
    const sp = 445;
    this.vx = damp(this.vx, ax * sp, 5.5, dt);
    this.vy = damp(this.vy, ay * sp, 5.5, dt);
    const dy = ay;                                    // aim (for fire below)
    if (Math.abs(this.vx) > 24) this.facing = this.vx < 0 ? -1 : 1;
  ...
  private drawDragon(game: Game, c: CanvasRenderingContext2D) {
    ...
    if (sheet && sheet.ready) {
      ...
      for (let k = 1; k < pts.length; k++) {
        ...
        c.lineWidth = 16 * t + 3;
        c.beginPath(); c.moveTo(pts[k - 1].x, pts[k - 1].y); c.lineTo(pts[k].x, pts[k].y); c.stroke();
      }
      c.restore();
      ...
      for (let g = 1; g <= 2; g++) {
        ...
        c.translate(p.x - cam.x, p.y - cam.y); c.rotate(this.dragonBank * this.facing); c.scale(this.facing, 1);
        sheet.blit(c, sheet.frameAt(this.animTime), SZ, false); c.restore();
      }
      // the dragon itself — the full coiled Zhulong sprite, banked to the flight angle
      c.save(); c.translate(hx, hy); c.rotate(this.dragonBank * this.facing); c.scale(this.facing, 1);
      c.shadowColor = '#ff8b3a'; c.shadowBlur = 26;
  ...
  draw(game: Game, c: CanvasRenderingContext2D) {
    ...
    if (!sheet?.ready) sheet = sprites.get('player/' + (this.animName === 'walk' ? 'run' : this.animName === 'fire' ? 'attack' : this.animName));
    if (!sheet?.ready) sheet = sprites.get('player/idle');
    if (sheet && sheet.ready) {
  ...
  private drawShotFx(game: Game, c: CanvasRenderingContext2D) {
    ...
    if (this.charging && this.chargeT > 0.12) {
      ...
      g.addColorStop(0, cr >= 1 ? '#ffe08a' : '#ff9d4d'); g.addColorStop(1, 'rgba(0,0,0,0)');
      c.fillStyle = g; c.beginPath(); c.arc(gx, gy, rad, 0, Math.PI * 2); c.fill(); c.restore();
    }
    if (this.attackTimer > 0) {
      const t = this.attackTimer / 0.24;
      const mx = cx + dx * 16, my = cy + dy * 12;
      c.save(); c.globalCompositeOperation = 'lighter'; c.globalAlpha = t;
      c.translate(mx, my); c.rotate(Math.atan2(dy, dx));
      c.fillStyle = game.world === 'day' ? '#ffe7a7' : '#bbe7ff';
      // elongated flash pointing exactly the way you're firing
      c.beginPath(); c.moveTo(0, -3 - t * 2); c.lineTo(20 * t + 8, 0); c.lineTo(0, 3 + t * 2); c.closePath(); c.fill();
      c.beginPath(); c.arc(0, 0, 4 + t * 4, 0, Math.PI * 2); c.fill();
      c.restore(); c.globalAlpha = 1;
    }
  }
}
