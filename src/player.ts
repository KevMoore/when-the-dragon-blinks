...
export class Player {
  ...
  private fireBolt(game: Game) {
    ...
    this.shootCd = 0.16; this.attackTimer = 0.14;
  ...
  private drawDragon(game: Game, c: CanvasRenderingContext2D) {
    ...
    if (sheet && sheet.ready) {
      ...
      for (let k = 3; k < pts.length - 1; k += 3) {            // dorsal fin-spikes
        const t = 1 - k / pts.length; if (t < 0.2) continue;
        const a = pts[k - 1], b = pts[k], ddx = b.x - a.x, ddy = b.y - a.y, L = Math.hypot(ddx, ddy) || 1, s = 8 * t;
        c.fillStyle = '#ffd877';
        c.beginPath(); c.moveTo(b.x - ddx / L * 4, b.y - ddy / L * 4); c.lineTo(b.x - ddy / L * s, b.y + ddx / L * s); c.lineTo(b.x + ddx / L * 4, b.y + ddy / L * 4); c.closePath(); c.fill();
      }
      ...
      c.shadowColor = '#ff8b3a'; c.shadowBlur = 20;
