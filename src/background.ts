...
export function drawSky(game: Game, c: CanvasRenderingContext2D) {
  ...
  dg.addColorStop(0, '#f0954f'); dg.addColorStop(.42, '#7a2b47'); dg.addColorStop(1, '#180a17');
  c.globalAlpha = day; c.fillStyle = dg; c.fillRect(0, 0, LOGICAL_W, LOGICAL_H);
  // night gradient
  const ng = c.createLinearGradient(0, 0, 0, LOGICAL_H);
  ng.addColorStop(0, '#081633'); ng.addColorStop(.48, '#17132e'); ng.addColorStop(1, '#08050d');
  c.globalAlpha = 1 - day; c.fillStyle = ng; c.fillRect(0, 0, LOGICAL_W, LOGICAL_H);
  c.globalAlpha = 1;

  // sun / moon
  c.save(); c.translate(-game.camera.x * 0.04, 0);
  c.globalAlpha = day; c.fillStyle = '#ffd17c'; c.shadowColor = '#ffbd54'; c.shadowBlur = 54;
  c.beginPath(); c.arc(772, 92, 44, 0, Math.PI * 2); c.fill();
  c.globalAlpha = 1 - day; c.fillStyle = '#dff0ff'; c.shadowColor = '#a9d6ff'; c.shadowBlur = 40;
  c.beginPath(); c.arc(760, 96, 33, 0, Math.PI * 2); c.fill();
  // moon craters
  c.shadowBlur = 0; c.fillStyle = 'rgba(150,180,220,.5)';
  c.beginPath(); c.arc(752, 90, 6, 0, Math.PI * 2); c.arc(770, 104, 4, 0, Math.PI * 2); c.fill();
...
export function drawDragonEye(game: Game, c: CanvasRenderingContext2D, day: number) {
  ...
  c.globalAlpha = 0.5; c.fillStyle = 'rgba(255,210,120,.06)';
  c.beginPath(); c.ellipse(x, y, 180, 62, 0, 0, Math.PI * 2); c.fill();
  c.globalAlpha = 0.7;
  c.strokeStyle = day > 0.5 ? 'rgba(255,210,120,.7)' : 'rgba(141,202,255,.4)'; c.lineWidth = 5;
  c.beginPath(); c.ellipse(x, y, 140, 44 * (0.18 + openness * 0.82), 0, 0, Math.PI * 2); c.stroke();
  if (openness > 0.06) {
    c.globalAlpha = openness;
    c.shadowColor = '#ff5c38'; c.shadowBlur = 40;
    c.fillStyle = '#18060b'; c.beginPath(); c.ellipse(x, y, 36, 40 * openness, 0, 0, Math.PI * 2); c.fill();
    c.fillStyle = '#f04d34'; c.beginPath(); c.ellipse(x, y, 14, 40 * openness, 0, 0, Math.PI * 2); c.fill();
    c.fillStyle = '#ffd9a0'; c.beginPath(); c.arc(x - 5, y - 8 * openness, 4, 0, Math.PI * 2); c.fill();
  }
...
export function drawParallax(game: Game, c: CanvasRenderingContext2D) {
  ...
  for (let i = 0; i < 9; i++) {
    ...
    c.save(); c.globalAlpha = (1 - day) * 0.9 + day * 0.15;
    c.shadowColor = day > 0.5 ? '#ffcf7a' : '#ffb066'; c.shadowBlur = 16;
    c.fillStyle = day > 0.5 ? '#ffe0a0' : '#ff9d5a';
    c.fillRect(lx, ly, 7, 10); c.restore();
  }
  ...
  for (let i = -2; i < 10; i++) {
    ...
    c.beginPath(); c.moveTo(x + 40, 424); c.lineTo(x + 94, 384); c.lineTo(x + 148, 424); c.closePath(); c.fill();
  }
}

function tileSolidTop(c: CanvasRenderingContext2D, x: number, y: number, top: string, body: string) {
  c.fillStyle = body; c.fillRect(x, y, TILE, TILE);
  c.fillStyle = top; c.fillRect(x, y, TILE, 6);
  c.fillStyle = 'rgba(0,0,0,.18)'; c.fillRect(x, y + TILE - 4, TILE, 4);
  c.strokeStyle = 'rgba(255,255,255,.05)'; c.strokeRect(x + .5, y + .5, TILE - 1, TILE - 1);
}
...
export function drawTiles(game: Game, c: CanvasRenderingContext2D) {
  ...
  for (let y = y0; y <= y1; y++) for (let x = x0; x <= x1; x++) {
    const ch = game.tileAt(x, y); if (ch === '.') continue;
    const sx = x * TILE - game.camera.x, sy = y * TILE - game.camera.y;
    if (ch === '#') tileSolidTop(c, sx, sy, '#3e3742', '#2a242f');
    else if (ch === 'g') { tileSolidTop(c, sx, sy, '#5c8a3a', '#2a242f'); c.fillStyle = '#79ad4c'; for (let i = 0; i < 5; i++) c.fillRect(sx + i * 7 + 2, sy - 2 - (i % 2), 2, 4); }
    else if (ch === 'o') { c.fillStyle = '#6b533a'; c.fillRect(sx, sy, TILE, 8); c.fillStyle = '#8a6b45'; c.fillRect(sx, sy, TILE, 3); }
...
function drawStatePlatform(game: Game, c: CanvasRenderingContext2D, x: number, y: number, state: 'day' | 'night') {
  ...
  c.save();
  if (active) { c.shadowColor = state === 'day' ? '#ffcf7a' : '#a9d6ff'; c.shadowBlur = 12; }
...
function drawHazard(game: Game, c: CanvasRenderingContext2D, x: number, y: number, ch: string) {
  ...
  c.save();
  if (active && ch !== '^') { c.shadowColor = ch === 'F' ? '#ff7840' : '#8ed7ff'; c.shadowBlur = 14; }
  c.fillStyle = ch === 'F' ? '#ff7840' : ch === 'S' ? '#8ed7ff' : '#b8a6a6';
  const wob = active ? Math.sin(game.time * 8 + x) * 1.5 : 0;
  for (let i = 0; i < 4; i++) { c.beginPath(); c.moveTo(x + i * 8, y + TILE); c.lineTo(x + i * 8 + 4, y + 7 + wob); c.lineTo(x + i * 8 + 8, y + TILE); c.fill(); }
  c.restore(); c.globalAlpha = 1;
}
...
export function drawLighting(game: Game, c: CanvasRenderingContext2D) {
  ...
  if (night > 0.02) {
    ...
    for (const pr of game.projectiles) glow(pr.x - game.camera.x, pr.y - game.camera.y, 46, 'rgba(255,170,90,.7)', 0.7);
