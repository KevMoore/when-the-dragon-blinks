...
function paintSky(c: CanvasRenderingContext2D, game: Game, th: Theme, day: number, qcx: number, cy: number, storm: boolean) {
  ...
  c.save(); c.globalCompositeOperation = 'lighter'; c.globalAlpha = day;
...
export function drawForeground(game: Game, c: CanvasRenderingContext2D) {
  ...
  for (let i = first; i <= last; i++) {
    ...
    c.globalAlpha = 0.55; c.save(); c.translate(x, LOGICAL_H + h * 0.76); c.rotate(sway); c.drawImage(dark, -w / 2, -h, w, h); c.restore();
  }
  c.globalAlpha = 1;
  c.restore();
}
...
function drawSpan(game: Game, c: CanvasRenderingContext2D, th: Theme, a: number, b: number, camX: number, camY: number, bottom: number, topY: (x: number) => number | null) {
  ...
  const raw: { x: number; y: number }[] = [{ x: leftX, y: topY(a)! }];
  for (let x = a; x <= b; x++) raw.push({ x: x * TILE + TILE / 2 - camX, y: topY(x)! });
  raw.push({ x: rightX, y: topY(b)! });
  const pts: { x: number; y: number; cliff?: boolean }[] = [raw[0]];
  ...
  const traceTop = () => {
    ...
    for (let i = 1; i < pts.length; i++) {
      if (pts[i].cliff || pts[i - 1].cliff) { c.lineTo(pts[i].x, pts[i].y); continue; }   // hard edge at cliffs
  ...
  if (topImgRaw && propReady['terrain_top']) {
    ...
    for (let sx0 = wx0; sx0 < rightX; sx0 += step) {
      ...
      if (!flat) { c.save(); c.transform(1, k, 0, 1, sx, y0); }
...
let _vignette: HTMLCanvasElement | null = null;
