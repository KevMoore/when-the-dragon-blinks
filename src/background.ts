...
type Theme = { day: string[]; night: string[]; haze: string; hazeNight: string; ridge: string; ridgeNight: string; accent: string };
...
// ---- tiles -----------------------------------------------------------------
function tileSolidTop(c: CanvasRenderingContext2D, x: number, y: number, top: string, body: string, bodyDark: string) {
  const g = c.createLinearGradient(0, y, 0, y + TILE);
  g.addColorStop(0, body); g.addColorStop(1, bodyDark);
  c.fillStyle = g; c.fillRect(x, y, TILE, TILE);
  c.fillStyle = top; c.fillRect(x, y, TILE, 6);
  c.fillStyle = 'rgba(255,255,255,.06)'; c.fillRect(x, y + 6, TILE, 2);
  c.fillStyle = 'rgba(0,0,0,.22)'; c.fillRect(x, y + TILE - 4, TILE, 4);
  c.strokeStyle = 'rgba(0,0,0,.18)'; c.strokeRect(x + .5, y + .5, TILE - 1, TILE - 1);
}
...
export function drawTiles(game: Game, c: CanvasRenderingContext2D) {
  ...
  for (let y = y0; y <= y1; y++) for (let x = x0; x <= x1; x++) {
    ...
    if (ch === '#') tileSolidTop(c, sx, sy, cap ? '#463c50' : stoneBody, stoneBody, stoneDark);
    else if (ch === 'g') { tileSolidTop(c, sx, sy, cap ? grassTop : stoneBody, stoneBody, stoneDark); if (cap) { c.fillStyle = mixHex(grassTop, '#c8f08a', 0.4); for (let i = 0; i < 5; i++) c.fillRect(sx + i * 7 + 2, sy - 3 - (i % 2), 2, 5); } }
    else if (ch === 'o') { c.save(); c.shadowColor = 'rgba(0,0,0,.4)'; c.shadowBlur = 6; c.fillStyle = '#6b533a'; c.fillRect(sx, sy, TILE, 9); c.restore(); c.fillStyle = '#8a6b45'; c.fillRect(sx, sy, TILE, 3); c.fillStyle = 'rgba(0,0,0,.3)'; c.fillRect(sx, sy + 7, TILE, 2); }
