...
**Mobile** — Landscape only (portrait shows a rotate screen). **Left thumb = a drag joystick**: tilt to move, push **up** to aim shots up, push **down** to crouch (diagonals work — crouch-walk, aim-up-while-moving). **Right thumb**: blink (☯), shoot (✦, hold to charge), jump (▲). **Double jump = tap ▲ twice**; **Dash = double-tap the joystick.** Shrines auto-open on approach.
