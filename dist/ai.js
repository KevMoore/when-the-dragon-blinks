...
export function groundBrain() {
    const NEAR = 48;
    const speed = (c) => c.e.kind === 'crawler' ? 145 : c.e.kind === 'ghoul' ? (c.game.world === 'night' ? 84 : 56) : 74;
    const actions = [
    ...
    const actions = [
        ...
        { name: 'dive', cost: 1, pre: { positioned: true, threatened: false }, post: { attacked: true, positioned: false },
            run(c, dt) {
                ...
                c.e.x += (tx - centerX(c.e.rect())) / d * 300 * dt;
                c.e.y += (ty - centerY(c.e.rect())) / d * 300 * dt;
