...
function groundStep(c, desiredVx, dt, commit = false, airVx = Math.abs(desiredVx)) {
    ...
    if (dir !== 0 && e.grounded) {
        ...
        if (!groundAhead && !stepBlocked) {
            ...
            let landK = 0;
            for (let k = 2; k <= 4; k++)
                if (game.overlapsSolid({ x: frontX + dir * k * TILE, y: foot - 6, w: TILE * 0.8, h: 60 })) {
                    landK = k;
                    break;
                }
            if (landK) {
                ...
                if (needV <= 680)
                    e.vy = Math.min(e.vy, -needV); // leap it
                else if (!groundBelow)
                    desiredVx = 0; // too far to clear and bottomless — hold
            }
            else if (!groundBelow)
                desiredVx = 0; // bottomless & nothing across — hold the edge
            else if (!commit && c.dy <= 40)
                desiredVx = 0; // survivable drop, but only take it when committed or chasing the player down
        }
        else if (!commit) {
            if (stepBlocked && !tallBlocked && headClear)
                e.vy = Math.min(e.vy, -300); // hop a low ledge
            else if (stepBlocked && tallBlocked)
                e.vy = Math.min(e.vy, -470); // full jump over a tall obstacle
        }
    }
    e.vx = desiredVx;
    e.vy += GRAVITY * dt;
    ...
    const actions = [
        ...
        { name: 'dive', cost: 1, pre: { positioned: true, threatened: false }, post: { attacked: true, positioned: false },
            run(c, dt) {
                ...
                if (c.bb.dt < 0.28) {
                    c.e.y += Math.sin(c.bb.dt * 40) * 14 * dt;
                    return false;
                }
                ...
                if (c.bb.dt > 0.85 || d < NEAR) {
                    c.bb.dt = 0;
                    c.bb.side = undefined;
                    c.bb.positioned = false;
                    c.bb.cool = 0.9 + Math.random() * 0.7;
                    return true;
                }
                return false;
            } },
