...
export class Player {
    ...
    drawSummon(game, c) {
        ...
        if (sheet && sheet.ready) {
            ...
            c.shadowBlur = 22;
            sheet.blit(c, frame, this.h * 1.72 * game.spriteScale, true);
            c.restore();
        }
    }
    drawDragon(game, c) {
    ...
    hurt(game, amount = 1, pit = false) {
        ...
        this.invuln = 1.1;
        game.combo = 0;
        game.comboT = 0; // taking a hit breaks the score combo
    ...
    draw(game, c) {
        if (game.deathT > 0)
            return; // the fragment has scattered — death fx draws instead
