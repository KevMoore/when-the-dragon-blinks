...
export class GuqinGame {
    ...
    get hitY() { return LOGICAL_H - 132; }
    get laneW() { return 92; }
    laneX(l) { const total = 4 * this.laneW + 3 * 16; return (LOGICAL_W - total) / 2 + l * (this.laneW + 16); }
    ...
    update(dt) {
        ...
        if (this.done) {
            ...
            if (this.finishT > 0.7 && (input.just('confirm') || input.pointer?.clicked))
                this.game.finishGuqin();
            return;
        }
        this.t += dt;
        for (let i = 0; i < 4; i++)
            this.hitFlash[i] = Math.max(0, this.hitFlash[i] - dt * 4);
        ...
        while (this.spawnIdx < this.melody.length) {
            ...
            if (this.t >= arriveT - this.fall) {
                this.tiles.push({ lane: note.lane, y: topY, hit: false, missed: false, fx: 0 });
                this.spawnIdx++;
            }
            else
                break;
        ...
        for (const tile of this.tiles) {
            ...
            tile.y += speed * dt;
            if (!tile.missed && tile.y > this.hitY + 48) {
                tile.missed = true;
                this.misses++;
                this.combo = 0;
            }
        ...
        for (let l = 0; l < 4; l++)
            if (this.laneTriggered(l))
                this.strike(l);
        if (this.spawnIdx >= this.melody.length && this.tiles.every(t => t.hit || t.missed))
            this.finish();
    }
    laneTriggered(l) {
        const input = this.game.input;
        if (input.pressed.has(LANE_KEY[l]))
            return true;
        const p = input.pointer;
        if (p && p.clicked) {
            const x = this.laneX(l);
            if (p.x >= x && p.x <= x + this.laneW && p.y > this.hitY - 260)
                return true;
        }
        return false;
    }
    strike(l) {
        ...
        for (const tile of this.tiles) {
            if (tile.lane !== l || tile.hit || tile.missed)
                continue;
            const d = Math.abs(tile.y - this.hitY);
            if (d < bestD) {
                bestD = d;
                best = tile;
            }
        ...
        if (best && bestD < 58) {
            ...
            this.game.audio.pluck(LANE_FREQ[l] * 0.5, 0.18);
            this.combo = 0; // dull thunk on a mistap
        }
    }
    finish() {
        if (this.done)
        ...
        if (this.game.score > this.game.save.highScore)
            this.game.save.highScore = this.game.score;
        this.game.audio.sfx('victory');
    }
    draw(c) {
        // dim backdrop
        c.fillStyle = 'rgba(10,5,12,0.82)';
        ...
        for (let l = 0; l < 4; l++) {
            ...
            c.strokeStyle = 'rgba(255,215,140,.14)';
            c.lineWidth = 1;
            c.strokeRect(x + .5, 84.5, w, hitY - 84 + 60);
            // string line
            c.strokeStyle = `rgba(255,230,180,${0.2 + this.hitFlash[l] * 0.6})`;
            c.lineWidth = 1 + this.hitFlash[l] * 2;
            c.beginPath();
            c.moveTo(x + w / 2, 84);
            c.lineTo(x + w / 2, hitY + 40);
            c.stroke();
        }
        // hit-line
        c.save();
        c.globalCompositeOperation = 'lighter';
        c.strokeStyle = 'rgba(255,200,110,.5)';
        c.lineWidth = 3;
        c.beginPath();
        c.moveTo(this.laneX(0) - 6, hitY);
        c.lineTo(this.laneX(3) + this.laneW + 6, hitY);
        c.stroke();
        c.restore();
        ...
        for (let l = 0; l < 4; l++) {
            ...
            roundRect(c, x + 6, hitY - 6, w - 12, 40, 8);
            c.fill();
            c.fillStyle = `rgba(255,245,220,${0.5 + fl * 0.5})`;
        ...
        for (const tile of this.tiles) {
            ...
            if (tile.hit) {
                ...
                c.globalAlpha = a;
                c.fillStyle = '#fff2c8';
                roundRect(c, x + 8 - tile.fx * 20, hitY - 8 - tile.fx * 30, w - 16 + tile.fx * 40, 44 + tile.fx * 30, 10);
                c.fill();
                c.globalAlpha = 1;
                continue;
            }
            if (tile.missed)
                continue;
            c.save();
            c.shadowColor = LANE_COL[tile.lane];
            c.shadowBlur = 14;
            const grad = c.createLinearGradient(0, tile.y, 0, tile.y + 44);
            grad.addColorStop(0, LANE_COL[tile.lane]);
            grad.addColorStop(1, mix(LANE_COL[tile.lane]));
            c.fillStyle = grad;
            roundRect(c, x + 8, tile.y, w - 16, 42, 10);
            c.fill();
            c.shadowBlur = 0;
            c.fillStyle = 'rgba(255,255,255,.22)';
            roundRect(c, x + 12, tile.y + 4, w - 24, 10, 5);
            c.fill();
            c.restore();
        }
        c.globalAlpha = 1;
        ...
        if (this.done) {
            c.fillStyle = 'rgba(8,4,10,0.7)';
            c.fillRect(0, LOGICAL_H / 2 - 90, LOGICAL_W, 180);
            c.textAlign = 'center';
            ...
            c.fillText(rank, LOGICAL_W / 2, LOGICAL_H / 2 - 30);
            c.fillStyle = '#fff2c8';
            c.font = '18px Georgia';
            ...
            if (this.finishT > 0.7) {
                c.fillStyle = `rgba(255,240,200,${0.5 + 0.4 * Math.sin(this.finishT * 4)})`;
                c.font = '14px Georgia';
                c.fillText('Press to continue', LOGICAL_W / 2, LOGICAL_H / 2 + 72);
            }
        }
        c.textAlign = 'left';
    }
