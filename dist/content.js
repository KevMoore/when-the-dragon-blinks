...
function buildLevel(spec, index) {
    ...
    for (let i = 0; i < gN; i++)
        gems.push(gem(Math.floor((i + 0.5) / gN * (w - 14)) + 6, gr - 2 - Math.floor(r() * 4)));
    // checkpoints
    const checkpoints = [];
    for (let cx = 34; cx < w - 12; cx += 44)
        checkpoints.push({ x: cx * TILE, y: gr * TILE - 40, w: 28, h: 56 });
    // moving / crumbling platforms
    const platforms = [];
    if (spec.moving)
        platforms.push(mp(Math.floor(w * 0.4), gr - 1, 3, { ax: 4 * TILE, speed: 0.8 + spec.diff * 0.12 }));
    if (spec.crumble) {
        platforms.push(mp(Math.floor(w * 0.55), gr - 1, 3, { crumble: true }));
        platforms.push(mp(Math.floor(w * 0.72), gr - 2, 3, { crumble: true }));
    }
