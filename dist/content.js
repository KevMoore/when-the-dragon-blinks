...
function ground(m, w, h, segs) {
    let si = 0;
    for (let x = 0; x < w; x++) {
        while (si + 1 < segs.length && segs[si + 1].x <= x)
            si++;
...
function buildLevel(spec, index) {
    ...
    for (let i = 0; i < gN; i++) {
        const gx = Math.floor((i + 0.5) / gN * (w - 14)) + 6;
        gems.push(gem(gx, topAt(segs, gx) - 2 - Math.floor(r() * 2)));
    }
    // checkpoints (on the LOCAL ground)
    const checkpoints = [];
    for (let cx = 34; cx < w - 12; cx += 44)
        checkpoints.push({ x: cx * TILE, y: topAt(segs, cx) * TILE - 40, w: 28, h: 56 });
    // moving / crumbling platforms — kept above the LOCAL ground so they never bury
    const platforms = [];
    if (spec.moving) {
        const px = Math.floor(w * 0.4);
        platforms.push(mp(px, topAt(segs, px) - 3, 3, { ax: 4 * TILE, speed: 0.8 + spec.diff * 0.12 }));
    }
    if (spec.crumble) {
        const p1 = Math.floor(w * 0.55), p2 = Math.floor(w * 0.72);
        platforms.push(mp(p1, topAt(segs, p1) - 3, 3, { crumble: true }));
        platforms.push(mp(p2, topAt(segs, p2) - 4, 3, { crumble: true }));
    }
    ...
    for (let ci = 0; ci < climbs; ci++) {
        ...
        for (let k = 0; k < tall; k++) {
            ...
            if (k === tall - 1)
                gems.push(gem(px + 1, py - 2));
            else if (r() < 0.42)
                entities.push({ kind: spec.pal[Math.floor(r() * spec.pal.length)], x: (px + 1) * TILE, y: (py - 2) * TILE });
        }
    }
    const bridges = [];
...
const ACT_THEME = { 1: 'mountain', 2: 'bridge', 3: 'cavern', 4: 'sunless' };
const ACT_SHRINE = { 1: 'shrine-who-is-zhulong', 2: 'shrine-eye-day-night', 3: 'shrine-breath', 4: 'shrine-sunless' };
const NAMES = [
...
function makeLevel1() {
    ...
    row(m, 80, 7, 6, 'N');
    row(m, 40, 13, 4, 'F');
    row(m, 108, 13, 4, 'S'); // stateful hazards on the ground
    return {
...
function makeLevel3() {
    ...
    row(m, 52, 5, 3, 'o');
    rect(m, 55, 3, 4, 2, '#'); // relic ledge atop shaft 1
...
export const levels = SPECS.map((s, i) => buildLevel(s, i));
        ...
        for (const f of files) {
            try {
                ...
                if (d && Array.isArray(d.tiles) && d.spawn && d.exit) {
                    ...
                    d.title = d.title || f;
                    d.subtitle = d.subtitle || 'A custom shrine path';
                    if (typeof d.replaces === 'number' && d.replaces >= 0 && d.replaces < 24) {
                        levels[d.replaces] = d; // designer edit of a campaign level — swap in place, keep unlock flow
                    }
                    else {
