...
export class LanternGame {
    ...
    lanternX(i) { return LOGICAL_W / 2 + (i - (N - 1) / 2) * 150; }
    lanternY() { return LOGICAL_H / 2 - 10; }
    get lr() { return 40; }
    nextRound() {
        this.round++;
    ...
    update(dt) {
        ...
        for (let i = 0; i < N; i++)
            this.lit[i] = Math.max(0, this.lit[i] - dt * 3.2);
        if (this.phase === 'done') {
            this.finishT += dt;
            if (this.finishT > 0.7 && (input.just('confirm') || input.pointer?.clicked))
                this.game.finishLantern();
            return;
        }
        if (this.phase === 'show') {
            this.timer -= dt;
            if (this.timer <= 0) {
                this.showStep++;
                if (this.showStep >= this.seq.length) {
                    this.phase = 'input';
                }
                else {
                    const l = this.seq[this.showStep];
                    this.lit[l] = 1;
                    this.game.audio.pluck(FREQ[l]);
                    this.timer = 0.62;
                }
            }
            return;
    ...
    tappedLantern() {
        ...
        for (let i = 0; i < N; i++)
            if (input.pressed.has(keys[i]))
                return i;
        const p = input.pointer;
        if (p && p.clicked)
            for (let i = 0; i < N; i++) {
                const dx = p.x - this.lanternX(i), dy = p.y - this.lanternY();
                if (dx * dx + dy * dy < (this.lr + 14) ** 2)
                    return i;
            }
        return -1;
    }
    ...
    draw(c) {
        ...
        c.fillText('燈 · The Lantern Rite', LOGICAL_W / 2, 52);
        ...
        for (let i = 0; i < N; i++) {
            ...
            // string
            c.strokeStyle = 'rgba(180,140,90,.4)';
            c.lineWidth = 1;
            c.beginPath();
            c.moveTo(x, ly - this.lr - 40);
            c.lineTo(x, ly - this.lr);
            c.stroke();
            // glow
            if (lit > 0.02) {
                c.save();
                c.globalCompositeOperation = 'lighter';
                c.globalAlpha = lit * 0.8;
                const g = c.createRadialGradient(x, ly, 0, x, ly, this.lr * 2.2);
                g.addColorStop(0, HUE[i]);
                g.addColorStop(1, 'rgba(0,0,0,0)');
                c.fillStyle = g;
                c.beginPath();
                c.arc(x, ly, this.lr * 2.2, 0, Math.PI * 2);
                c.fill();
                c.restore();
            }
            // lantern body
            c.save();
            c.globalAlpha = 0.4 + lit * 0.6;
            const bg = c.createLinearGradient(x, ly - this.lr, x, ly + this.lr);
            bg.addColorStop(0, HUE[i]);
            bg.addColorStop(1, mixDark(HUE[i]));
            c.fillStyle = bg;
            c.strokeStyle = 'rgba(30,12,10,.7)';
            c.lineWidth = 2;
            c.beginPath();
            c.ellipse(x, ly, this.lr * 0.8, this.lr, 0, 0, Math.PI * 2);
            c.fill();
            c.stroke();
            c.fillStyle = 'rgba(30,12,10,.8)';
            c.fillRect(x - this.lr * 0.5, ly - this.lr - 5, this.lr, 5);
            c.fillRect(x - this.lr * 0.5, ly + this.lr, this.lr, 5);
            // tassel
            c.strokeStyle = '#c8302a';
            c.lineWidth = 2;
            c.beginPath();
            c.moveTo(x, ly + this.lr + 5);
            c.lineTo(x, ly + this.lr + 18);
            c.stroke();
            c.restore();
        }
        ...
        c.fillStyle = 'rgba(255,215,140,.8)';
        c.fillText(`Bonus ${this.bonus}`, 30, 132);
        if (this.phase === 'done') {
            c.fillStyle = 'rgba(8,4,10,0.7)';
            c.fillRect(0, LOGICAL_H / 2 + 70, LOGICAL_W, 130);
            c.textAlign = 'center';
            c.fillStyle = '#ffd777';
            c.font = 'bold 30px Georgia';
            ...
            c.fillText(`+${this.bonus} bonus`, LOGICAL_W / 2, LOGICAL_H / 2 + 146);
            if (this.finishT > 0.7) {
                c.fillStyle = `rgba(255,240,200,${0.5 + 0.4 * Math.sin(this.finishT * 4)})`;
                c.font = '14px Georgia';
                c.fillText('Press to continue', LOGICAL_W / 2, LOGICAL_H / 2 + 174);
            }
        }
        c.textAlign = 'left';
    }
