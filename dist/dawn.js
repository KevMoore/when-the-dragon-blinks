...
export class DawnGame {
    ...
    get halfWin() { return 0.15 - this.round * 0.022; } // dawn window shrinks each round
    beginRound() {
        this.pos = 0;
        this.dir = 1;
        this.stopped = false;
    ...
    draw(c) {
        ...
        c.fillText('黎明 · Catch the Dawn', cx, 52);
        ...
        c.save();
        c.strokeStyle = 'rgba(255,200,120,.5)';
        c.lineWidth = 3;
        c.beginPath();
        c.ellipse(cx, cy, 120, 20 + 62 * open, 0, 0, Math.PI * 2);
        c.stroke();
        if (open > 0.03) {
            c.save();
            c.globalCompositeOperation = 'lighter';
            const g = c.createRadialGradient(cx, cy, 2, cx, cy, 70);
            g.addColorStop(0, `rgba(255,240,200,${open})`);
            g.addColorStop(0.4, `rgba(240,90,44,${open * 0.8})`);
            g.addColorStop(1, 'rgba(0,0,0,0)');
            c.fillStyle = g;
            c.beginPath();
            c.ellipse(cx, cy, 60, (20 + 62 * open) * 0.9, 0, 0, Math.PI * 2);
            c.fill();
            c.restore();
            c.fillStyle = '#1a0509';
            c.beginPath();
            c.ellipse(cx, cy, 22, (10 + 40 * open), 0, 0, Math.PI * 2);
            c.fill();
            c.fillStyle = '#ff5230';
            c.beginPath();
            c.ellipse(cx, cy, 10, (10 + 40 * open) * 0.6, 0, 0, Math.PI * 2);
            c.fill();
        }
        c.restore();
        // the sweep bar
        ...
        c.save();
        c.globalCompositeOperation = 'lighter';
        c.fillStyle = 'rgba(255,200,110,.5)';
        roundRect(c, bx + bw * (0.5 - hw), by, bw * hw * 2, 20, 10);
        c.fill();
        c.restore();
        c.fillStyle = 'rgba(255,240,200,.4)';
        c.fillRect(cx - 1, by - 4, 2, 28); // centre tick
        // marker
        const mx = bx + bw * this.pos;
        c.save();
        c.shadowColor = '#ffd777';
        c.shadowBlur = 12;
        c.fillStyle = this.stopped ? (this.lastQuality > 0 ? '#ffe6a0' : '#c8503a') : '#fff2c8';
        c.beginPath();
        c.moveTo(mx, by - 8);
        c.lineTo(mx + 8, by - 20);
        c.lineTo(mx - 8, by - 20);
        ...
        if (this.stopped && !this.done) {
            c.textAlign = 'center';
            c.fillStyle = this.lastQuality > 0 ? '#ffd777' : 'rgba(200,200,220,.7)';
            c.font = 'bold 24px Georgia';
            c.fillText(this.lastQuality > 0.7 ? '完美 · Perfect dawn!' : this.lastQuality > 0 ? '好 · Caught' : '暗 · Missed', cx, by - 40);
        }
        if (this.done) {
            c.fillStyle = 'rgba(8,4,10,0.7)';
            c.fillRect(0, cy - 60, LOGICAL_W, 150);
            c.textAlign = 'center';
            c.fillStyle = '#ffd777';
            ...
            c.fillText(`+${this.bonus} bonus`, cx, cy + 34);
            if (this.finishT > 0.7) {
                c.fillStyle = `rgba(255,240,200,${0.5 + 0.4 * Math.sin(this.finishT * 4)})`;
                c.font = '14px Georgia';
                c.fillText('Press to continue', cx, cy + 66);
            }
        }
        c.textAlign = 'left';
    }
