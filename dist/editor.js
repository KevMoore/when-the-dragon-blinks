...
const toolDef = (id) => TOOLS.find(t => t && t.id === id);
// ---- state ---------------------------------------------------------------------
...
function undo() { const s = undoStack.pop(); if (!s)
    return; redoStack.push(JSON.stringify(st)); st = JSON.parse(s); H = st.h || 17; syncInspector(); save(); }
function redo() { const s = redoStack.pop(); if (!s)
    return; undoStack.push(JSON.stringify(st)); st = JSON.parse(s); H = st.h || 17; syncInspector(); save(); }
...
function toolLabel(id) { return STAMPS.find(s => s[0] === id)?.[2]?.split(' — ')[0] ?? toolDef(id)?.label ?? id; }
...
function ask(title, body, yes = 'Confirm') {
    return new Promise(res => {
        ...
        const done = (v) => { ov.classList.remove('show'); $('confirm-yes').onclick = null; $('confirm-no').onclick = null; res(v); };
...
function syncInspector() {
    const insp = $('insp');
    insp.innerHTML = '';
    const sec = (title) => { const d = document.createElement('div'); d.className = 'sec'; d.innerHTML = `<h3>${title}</h3>`; insp.appendChild(d); return d; };
    // LEVEL
    const lv = sec('Level');
    ...
    $('wtiles').value = String(st.w);
    $('name').oninput = e => { st.name = e.target.value; save(); };
    $('theme').onchange = e => { st.theme = e.target.value; save(); };
    $('wtiles').onchange = e => {
    ...
    if (st.replaces !== undefined) {
        ...
        $('detach').onclick = () => { st.replaces = undefined; delete st.carry?.id; syncInspector(); save(); toast('Detached — will publish as a new Custom Trail'); };
    }
    ...
    if (['Enemy', 'Spawner', 'pack'].includes(tool)) {
        ...
        selE.onchange = () => { enemyKind = selE.value; };
        ...
        if (tool !== 'Spawner') {
            ...
            tg.className = 'toggle' + (elite ? ' on' : '');
            tg.textContent = elite ? '★ elite' : '☆ elite';
            ...
            $('spevery').onchange = e => { spEvery = Math.max(1, parseInt(e.target.value) || 4); };
            $('spmax').onchange = e => { spMax = Math.max(1, parseInt(e.target.value) || 3); };
        }
    }
    // STAMPS
    ...
    // FILE
    const ac = sec('File');
    ac.classList.add('actions');
    const mkBtn = (label, fn, cls = 'tbtn') => { const b = document.createElement('button'); b.className = cls; b.textContent = label; b.onclick = fn; ac.appendChild(b); };
    mkBtn('🗒 New level', async () => {
        if (!(await ask('Start a new level?', 'Your current draft stays in autosave until you paint over it.', 'New level')))
            return;
        pushUndo();
        H = 17;
        st = freshState(parseInt($('wtiles').value) || 120);
        syncInspector();
        save();
        toast('Fresh canvas ready');
    });
    mkBtn('⬆️ Import JSON', () => $('file').click());
    mkBtn('⬇️ Export JSON', () => {
        ...
        a.click();
        toast('Level exported');
    });
    updateInfo();
}
let campaignLevels = null;
async function populateCampaign() {
    const campSel = $('campaign');
    try {
        if (!campaignLevels) {
            const m = await import('./content.js');
            campaignLevels = m.levels;
        }
        campaignLevels.slice(0, 24).forEach((lv, i) => {
            const o = document.createElement('option');
            o.value = String(i);
        ...
        campSel.onchange = async () => {
            ...
            if (isNaN(i))
                return;
            if (!(await ask(`Load L${i + 1} for editing?`, 'Publishing this level will REPLACE it in the live game (you can Detach to publish as a new level instead).', 'Load it')))
                return;
            pushUndo();
...
$('publish').onclick = () => {
    $('gh-token').value = localStorage.getItem('wtdb-gh-token') || '';
...
$('file').onchange = e => {
    ...
    f.text().then(t => { try {
        ...
        toast('✗ Not a valid level JSON');
    } });
};
// ---- zoom / pan ------------------------------------------------------------------
function setZoom(z, cx = cv.width / 2, cy = cv.height / 2) {
...
const isStamp = (id) => STAMPS.some(s => s[0] === id);
...
window.addEventListener('keyup', e => { if (e.key === ' ')
    spaceHeld = false; });
...
function apply(p) {
    ...
    if (tool === 'Spawn')
        ...
        st.spawners.push({ kind: enemyKind, ...p, every: spEvery, max: spMax });
    else if (tool === 'Bridge') {
        if (!bridgeStart) {
            bridgeStart = { ...p };
            updateHint('Now click the bridge END');
        }
        else {
...
function draw() {
    ...
    if (zoom > 0.4) {
        ...
        for (let y = 0; y <= H; y++) {
            c.moveTo(vx0 * TILE, y * TILE);
            c.lineTo((vx1 + 1) * TILE, y * TILE);
        }
        c.stroke();
    }
    for (const b of st.bridges) {
