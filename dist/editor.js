...
function buildTools() {
    ...
    toolsEl.appendChild(zo);
    const zl = document.createElement('button');
    zl.textContent = Math.round(zoom * 100) + '%';
    zl.title = 'Reset zoom';
    zl.onclick = () => setZoom(1);
    toolsEl.appendChild(zl);
    const zi = document.createElement('button');
    zi.textContent = '+';
    zi.title = 'Zoom in';
    zi.onclick = () => setZoom(zoom * 1.25);
    toolsEl.appendChild(zi);
    const zf = document.createElement('button');
    zf.textContent = '⤢ Fit';
    zf.title = 'Fit the whole level';
    zf.onclick = fitZoom;
    toolsEl.appendChild(zf);
}
