...
function resize() {
    ...
    if (!(vw > 0 && vh > 0))
        return; // can report 0 mid-rotation — keep the previous geometry
...
(() => {
    ...
    if (!isIOS || installed || !vp)
        return;
    window.__islActive = true;
    ...
    const apply = (o) => {
        ...
        if (typeof resize === 'function')
            resize(); // refit the canvas to the new viewport height
    };
    const measure = () => {
        if (rafId)
...
(() => {
    ...
    let dismissed = false;
    try {
        dismissed = localStorage.getItem('wtdb-install-dismissed') === '1';
    }
    catch { /* private mode */ }
    if (!coarse || installed || dismissed)
        return;
    const ua = navigator.userAgent;
    ...
    const remember = () => { try {
        localStorage.setItem('wtdb-install-dismissed', '1');
    }
    catch { /* ignore */ } };
...
function loop(now) {
    try {
        ...
        if (++frameErrors <= 10 || frameErrors % 300 === 0)
            console.error('[dragon] frame error', err);
        if (frameErrors > 1800)
            return; // ~30s of failing every frame — stop burning battery
    }
...
window.addEventListener('resize', () => { try {
    ...
    console.error('[dragon] resize render', err);
} });
