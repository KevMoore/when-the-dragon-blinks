...
function oneOf(items) { return items[Math.floor(Math.random() * items.length)]; }
...
function rectTiles(map, x, y, w, h, c) {
    for (let yy = y; yy < y + h; yy++)
        for (let xx = x; xx < x + w; xx++)
            setTile(map, xx, yy, c);
}
function mapToStrings(map) { return map.map(row => row.join('')); }
function makeLevel1() {
    const w = 76, h = 17;
...
class Input {
    ...
    down(action) {
        ...
        if (action === 'pause')
            return k.has('escape') || !!this.gpButtons[9];
        if (action === 'interact')
            return k.has('f') || k.has('arrowup') || !!this.gpButtons[0];
        return false;
    }
    just(action) {
        ...
        if (action === 'pause')
            return p.has('escape') || gpJust(9);
        if (action === 'interact')
            return p.has('f') || gpJust(0);
        if (action === 'debug')
            return p.has('f1');
        if (action === 'codex')
            return p.has('h');
...
class AudioManager {
    ...
    beep(type) {
        ...
        try {
            ...
            if (type === 'toggle')
                osc.frequency.exponentialRampToValueAtTime(freq * 2.2, now + len);
...
class Player {
    constructor() {
        ...
        this.hp = 5;
        this.maxHp = 5;
        this.checkpoint = { x: 64, y: 410 };
        this.attackTimer = 0;
        this.attackCooldown = 0;
        this.landDustCooldown = 0;
    }
    rect() { return { x: this.x, y: this.y, w: this.w, h: this.h }; }
    attackRect() { return { x: this.facing > 0 ? this.x + this.w - 2 : this.x - 30, y: this.y + 7, w: 32, h: 24 }; }
    reset(level) {
    ...
    update(game, dt) {
        ...
        if (!left && !right)
            this.vx = lerp(this.vx, 0, this.grounded ? 0.22 : 0.08);
        ...
        if (!input.down('jump') && this.vy < -130)
            this.vy += 1600 * dt;
        ...
        for (const zone of game.level.windZones || []) {
            if (overlap(this.rect(), zone)) {
                ...
                if (Math.random() < 0.5)
                    game.particles.push({ x: rand(zone.x, zone.x + zone.w), y: rand(zone.y + zone.h - 20, zone.y + zone.h), vx: rand(-15, 15), vy: rand(-120, -60), life: .8, maxLife: .8, size: rand(2, 5), kind: 'mist' });
            }
        }
        ...
        if (this.y > game.level.height * TILE + 200)
    ...
    draw(game, c) {
        ...
        c.beginPath();
        c.ellipse(0, 25, 16, 5, 0, 0, Math.PI * 2);
        c.fill();
        // robe
        ...
        if (this.attackTimer > 0) {
            ...
            c.strokeStyle = game.world === 'day' ? '#ffe7a7' : '#bbe7ff';
            c.lineWidth = 4;
            c.beginPath();
            c.arc(25, -2, 22, -1.2, 1.2);
            c.stroke();
        }
        c.restore();
        c.globalAlpha = 1;
...
class Enemy {
    ...
    update(game, dt) {
        ...
        if (this.kind === 'moth') {
            ...
            if (active && Math.abs(dx) < 360) {
                this.vx = lerp(this.vx, Math.sign(dx) * 95, 0.04);
                this.vy = lerp(this.vy, Math.sign(dy) * 60, 0.03);
            }
            else {
                this.vx = Math.sin(game.time * 1.8 + this.phase) * 35;
                this.vy = Math.cos(game.time * 2.2 + this.phase) * 20;
            }
            this.x += this.vx * dt;
            this.y += this.vy * dt;
        }
        if (this.kind === 'wisp') {
            ...
            if (active)
                this.x += Math.sin(game.time + this.phase) * 36 * dt;
        }
    ...
    draw(game, c) {
        ...
        if (this.kind === 'moth') {
            ...
            c.beginPath();
            c.ellipse(-8, 0, 13, 8 + Math.sin(game.time * 18) * 3, .4, 0, Math.PI * 2);
            c.fill();
            c.beginPath();
            c.ellipse(8, 0, 13, 8 - Math.sin(game.time * 18) * 3, -.4, 0, Math.PI * 2);
            c.fill();
...
class LanternEater {
    ...
    rect() { return { x: this.x + 22, y: this.y + 34, w: this.w - 44, h: this.h - 44 }; }
    update(game, dt) {
        if (!this.alive)
            return;
        this.timer += dt;
        ...
        if (overlap(this.rect(), game.player.rect()))
    ...
    attack(game) {
        ...
        if (this.phase >= 2 && Math.random() < .45)
    ...
    draw(game, c) {
        ...
        c.shadowColor = this.vulnerable ? '#a9d6ff' : '#ff8b44';
        c.shadowBlur = this.vulnerable ? 28 : 18;
        const body = c.createRadialGradient(0, 0, 10, 0, 0, 78);
        body.addColorStop(0, this.vulnerable ? '#345070' : '#4a171c');
        body.addColorStop(1, '#08050d');
        c.fillStyle = body;
        c.beginPath();
        c.moveTo(-45, -48);
        c.bezierCurveTo(-85, -10, -65, 75, 0, 75);
        c.bezierCurveTo(70, 75, 85, -20, 45, -48);
        c.bezierCurveTo(24, -72, -22, -72, -45, -48);
        c.fill();
        c.shadowBlur = 0;
        // lantern mask
        c.fillStyle = this.vulnerable ? '#e7f8ff' : '#f0b752';
        c.beginPath();
        c.roundRect(-36, -42, 72, 54, 18);
        c.fill();
        c.fillStyle = '#150910';
        c.fillRect(-25, -22, 18, 8);
        c.fillRect(7, -22, 18, 8);
        c.fillStyle = this.vulnerable ? '#81d9ff' : '#d94a3a';
        c.beginPath();
        c.arc(0, 5, this.vulnerable ? 13 : 8, 0, Math.PI * 2);
        c.fill();
        // smoky arms
        ...
        c.beginPath();
        c.moveTo(-42, 10);
        c.bezierCurveTo(-92, 20, -95, 64, -58, 78);
        c.moveTo(42, 10);
        c.bezierCurveTo(92, 20, 95, 64, 58, 78);
        c.stroke();
        c.restore();
...
class Game {
    ...
    unlockCodex(ids) { for (const id of ids)
        if (!this.save.codex.includes(id))
            this.save.codex.push(id); this.persist(); }
    ...
    flashText(text) { if (text)
        this.message = { text, t: 0, max: 2.5 };
    else
        this.message = null; }
    tileAt(tx, ty) {
    ...
    isSolidChar(ch, world = this.world) { return ch === '#' || (ch === 'D' && world === 'day') || (ch === 'N' && world === 'night'); }
    isHazardChar(ch) { return ch === '^' || (ch === 'F' && this.world === 'day') || (ch === 'S' && this.world === 'night'); }
    solidsForRect(rect, world = this.world) {
        const out = [];
        ...
        for (let y = y0; y <= y1; y++)
            for (let x = x0; x <= x1; x++) {
                ...
                if (this.isSolidChar(ch, world))
                    out.push({ x: x * TILE, y: y * TILE, w: TILE, h: TILE });
            }
        return out;
    }
    overlapsSolid(rect, world = this.world) { return this.solidsForRect(rect, world).some(s => overlap(rect, s)); }
    moveEntity(e, dx, dy) {
    ...
    updateTitle() {
        ...
        if (this.input.just('levelselect')) {
            this.state = 'levelSelect';
            return;
        }
        if (this.input.pointer?.clicked) {
            const y = this.input.pointer.y;
            options.forEach((_, i) => { if (y > 334 + i * 44 && y < 372 + i * 44)
    ...
    updateLevelSelect() {
        ...
        if (this.input.just('left'))
            this.levelSelection = clamp(this.levelSelection - 1, 0, Math.min(this.save.highestUnlocked, levels.length - 1));
        if (this.input.just('right'))
            this.levelSelection = clamp(this.levelSelection + 1, 0, Math.min(this.save.highestUnlocked, levels.length - 1));
        if (this.input.pointer?.clicked) {
            const x = this.input.pointer.x;
            levels.forEach((_, i) => { if (x > 110 + i * 190 && x < 270 + i * 190)
                this.levelSelection = i; });
            if (this.levelSelection <= this.save.highestUnlocked)
    ...
    updateCodex() {
        ...
        if (this.input.just('up'))
            this.codexSelection = (this.codexSelection + entries.length - 1) % entries.length;
        if (this.input.just('down'))
            this.codexSelection = (this.codexSelection + 1) % entries.length;
        if (this.input.pointer?.clicked) {
            const y = this.input.pointer.y;
            entries.forEach((_, i) => { if (y > 115 + i * 38 && y < 148 + i * 38)
                this.codexSelection = i; });
        }
    }
    updateLore() { if (this.input.just('confirm') || this.input.just('back') || this.input.pointer?.clicked)
        this.closeLore(); }
    updatePause() {
    ...
    checkHazardsAndObjects() {
        ...
        for (let y = y0; y <= y1; y++)
            for (let x = x0; x <= x1; x++) {
                if (this.isHazardChar(this.tileAt(x, y)) && overlap(pr, { x: x * TILE + 4, y: y * TILE + 4, w: TILE - 8, h: TILE - 8 }))
        ...
        for (const relic of this.level.relics) {
            ...
            if (overlap(pr, r)) {
                ...
                this.persist();
                this.spawnSparks(relic.x + 11, relic.y + 11, 24);
    ...
    updateProjectiles(dt) {
        for (const pr of this.projectiles) {
            ...
            pr.y += pr.vy * dt;
            pr.life -= dt;
            if (overlap(this.player.rect(), { x: pr.x - pr.r, y: pr.y - pr.r, w: pr.r * 2, h: pr.r * 2 })) {
                pr.life = 0;
                this.player.hurt(this);
            }
            if (this.overlapsSolid({ x: pr.x - pr.r, y: pr.y - pr.r, w: pr.r * 2, h: pr.r * 2 }))
                pr.life = 0;
        }
    ...
    updateParticles(dt) {
        for (const p of this.particles) {
            p.x += p.vx * dt;
            p.y += p.vy * dt;
            p.vy += (p.kind === 'dust' ? 160 : p.kind === 'mist' ? -10 : 80) * dt;
            p.life -= dt;
        }
        this.particles = this.particles.filter(p => p.life > 0);
        if (this.state === 'title' && Math.random() < .8)
            this.particles.push({ x: rand(0, LOGICAL_W), y: rand(130, 520), vx: rand(-10, 10), vy: rand(-8, -2), life: rand(2, 5), maxLife: 5, size: rand(1, 3), kind: 'star' });
    }
    spawnDust(x, y, n) { for (let i = 0; i < n; i++)
        this.particles.push({ x, y, vx: rand(-90, 90), vy: rand(-70, -10), life: rand(.25, .55), maxLife: .55, size: rand(2, 5), kind: 'dust' }); }
    spawnSparks(x, y, n) { for (let i = 0; i < n; i++) {
        const a = rand(0, Math.PI * 2), sp = rand(50, 220);
        this.particles.push({ x, y, vx: Math.cos(a) * sp, vy: Math.sin(a) * sp, life: rand(.25, .75), maxLife: .75, size: rand(2, 4), kind: 'spark' });
    } }
    spawnHit(x, y, n) { for (let i = 0; i < n; i++) {
        const a = rand(0, Math.PI * 2), sp = rand(80, 320);
        this.particles.push({ x, y, vx: Math.cos(a) * sp, vy: Math.sin(a) * sp, life: rand(.18, .65), maxLife: .65, size: rand(2, 6), kind: 'hit' });
    } }
    render() {
        ctx.save();
    ...
    drawWorld(c) {
        ...
        this.drawTiles(c);
        for (const cp of this.level.checkpoints)
            this.drawCheckpoint(c, cp);
        for (const shrine of this.level.shrines)
            this.drawShrine(c, shrine.x, shrine.y);
        for (const relic of this.level.relics)
            if (!this.save.relics.includes(relic.id))
                this.drawRelic(c, relic.x, relic.y);
        ...
        if (this.debug)
            this.drawDebug(c);
        if (this.message)
            this.drawFloatingText(c, this.message);
    }
    drawBackground(c) {
        ...
        dg.addColorStop(0, '#ef8a49');
        dg.addColorStop(.45, '#51213a');
        dg.addColorStop(1, '#150915');
        c.globalAlpha = dayAlpha;
        c.fillStyle = dg;
        c.fillRect(0, 0, LOGICAL_W, LOGICAL_H);
        const ng = c.createLinearGradient(0, 0, 0, LOGICAL_H);
        ng.addColorStop(0, '#07142d');
        ng.addColorStop(.48, '#16122b');
        ng.addColorStop(1, '#08050d');
        c.globalAlpha = nightAlpha;
        c.fillStyle = ng;
        c.fillRect(0, 0, LOGICAL_W, LOGICAL_H);
        c.globalAlpha = 1;
        // sun/moon
        c.save();
        c.translate(-this.camera.x * .04, 0);
        c.globalAlpha = dayAlpha;
        c.fillStyle = '#ffd17c';
        c.shadowColor = '#ffbd54';
        c.shadowBlur = 48;
        c.beginPath();
        c.arc(770, 90, 42, 0, Math.PI * 2);
        c.fill();
        c.globalAlpha = nightAlpha;
        c.fillStyle = '#d7eeff';
    ...
    drawDragonEye(c, dayAlpha) {
        c.save();
        const x = 220 - this.camera.x * .03, y = 92;
        c.globalAlpha = .55;
        c.fillStyle = 'rgba(255,210,120,.08)';
        c.beginPath();
        c.ellipse(x, y, 170, 58, .02, 0, Math.PI * 2);
        c.fill();
        c.globalAlpha = .72;
        c.strokeStyle = dayAlpha > .5 ? 'rgba(255,210,120,.75)' : 'rgba(141,202,255,.45)';
        c.lineWidth = 5;
        c.beginPath();
        c.ellipse(x, y, 132, 40 * (.25 + dayAlpha * .75), 0, 0, Math.PI * 2);
        c.stroke();
        if (dayAlpha > .08) {
            c.globalAlpha = dayAlpha;
            ...
            c.fillStyle = '#18060b';
            c.beginPath();
            c.ellipse(x, y, 34, 39, 0, 0, Math.PI * 2);
            c.fill();
            c.fillStyle = '#f04d34';
            c.beginPath();
            c.ellipse(x, y, 13, 39, 0, 0, Math.PI * 2);
            c.fill();
        }
        c.restore();
        c.globalAlpha = 1;
    ...
    drawTiles(c) {
        ...
        for (let y = y0; y <= y1; y++)
            for (let x = x0; x <= x1; x++) {
                ...
                if (ch === '#')
                    this.drawStone(c, sx, sy);
                if (ch === 'D')
                    this.drawStatePlatform(c, sx, sy, 'day');
                if (ch === 'N')
                    this.drawStatePlatform(c, sx, sy, 'night');
                if (ch === '^' || ch === 'F' || ch === 'S')
                    this.drawHazard(c, sx, sy, ch);
            }
    }
    drawStone(c, x, y) {
        c.fillStyle = '#2c2630';
    ...
    drawWind(c) {
        for (const z of this.level.windZones || []) {
            ...
            c.save();
            c.globalAlpha = this.world === 'day' ? .17 : .28;
            c.strokeStyle = this.world === 'day' ? '#ffe19a' : '#bfeeff';
            c.lineWidth = 2;
            for (let i = 0; i < 6; i++) {
                const y = sy + (i * 37 + this.time * 65) % z.h;
                c.beginPath();
                c.moveTo(sx + 15 + Math.sin(this.time * 2 + i) * 10, y);
                c.bezierCurveTo(sx + z.w / 2, y - 25, sx + z.w - 20, y + 20, sx + z.w - 5, y - 12);
                c.stroke();
            }
            c.restore();
        }
    ...
    drawCheckpoint(c, r) {
        ...
        c.fillStyle = '#ffd77d';
        c.beginPath();
        c.arc(x + 14, y + 16, 8, 0, Math.PI * 2);
        c.fill();
    }
    drawShrine(c, x, y) {
        const sx = x - this.camera.x, sy = y - this.camera.y;
        c.fillStyle = '#2b121d';
        c.fillRect(sx, sy, 26, 55);
        c.fillStyle = '#d6a348';
        c.fillRect(sx - 8, sy, 42, 8);
        c.fillStyle = this.world === 'day' ? '#ffd777' : '#a9d6ff';
        c.beginPath();
        c.arc(sx + 13, sy + 24, 7 + Math.sin(this.time * 5) * 1.5, 0, Math.PI * 2);
        c.fill();
        if (Math.abs(this.player.x - x) < 55 && Math.abs(this.player.y - y) < 70)
            this.drawSmallPrompt(c, sx - 18, sy - 24, 'F / ▲ Lore');
    }
    drawRelic(c, x, y) {
        ...
        c.beginPath();
        c.moveTo(sx + 11, sy);
        c.lineTo(sx + 22, sy + 11);
        c.lineTo(sx + 11, sy + 22);
        c.lineTo(sx, sy + 11);
        c.closePath();
        c.fill();
        c.restore();
    ...
    drawExit(c) {
        ...
        c.lineWidth = 5;
        c.beginPath();
        c.roundRect(sx, sy, e.w, e.h, 18);
        c.stroke();
        c.fillStyle = 'rgba(255,255,255,.08)';
        c.fillRect(sx + 9, sy + 10, e.w - 18, e.h - 20);
        c.restore();
    }
    drawProjectile(c, p) {
        const x = p.x - this.camera.x, y = p.y - this.camera.y;
        c.save();
        c.shadowColor = '#ff674d';
        c.shadowBlur = 18;
        c.fillStyle = '#ffb45d';
        c.beginPath();
    ...
    drawParticles(c) {
        for (const p of this.particles) {
            ...
            c.beginPath();
            c.arc(p.x - this.camera.x, p.y - this.camera.y, p.size, 0, Math.PI * 2);
            c.fill();
        }
        c.globalAlpha = 1;
    }
    drawSmallPrompt(c, x, y, text) {
        c.save();
        c.font = '14px Georgia';
        c.textAlign = 'center';
        c.fillStyle = 'rgba(0,0,0,.55)';
        c.fillRect(x - 38, y - 17, 76, 22);
        c.strokeStyle = 'rgba(246,191,94,.45)';
        c.strokeRect(x - 38, y - 17, 76, 22);
        c.fillStyle = '#fff1ca';
    ...
    drawHUD(c) {
        c.save();
        c.fillStyle = 'rgba(9,5,13,.58)';
        c.fillRect(18, 16, 322, 62);
        c.strokeStyle = 'rgba(246,191,94,.35)';
        c.strokeRect(18.5, 16.5, 322, 62);
        for (let i = 0; i < this.player.maxHp; i++) {
            c.fillStyle = i < this.player.hp ? '#d94a3a' : '#34212b';
            c.beginPath();
            c.moveTo(42 + i * 28, 36);
            c.bezierCurveTo(32 + i * 28, 22, 20 + i * 28, 37, 42 + i * 28, 55);
            c.bezierCurveTo(64 + i * 28, 37, 52 + i * 28, 22, 42 + i * 28, 36);
            c.fill();
        }
        const worldLabel = this.world === 'day' ? 'DAY: Eye Open' : 'NIGHT: Eye Closed';
        c.fillStyle = this.world === 'day' ? '#ffd777' : '#a9d6ff';
        c.font = '18px Georgia';
        c.fillText(worldLabel, 190, 40);
        c.font = '13px Georgia';
        c.fillStyle = 'rgba(255,255,255,.78)';
        c.fillText('E/C/Y toggles the dragon blink', 190, 61);
        c.fillStyle = 'rgba(9,5,13,.58)';
        c.fillRect(LOGICAL_W - 308, 16, 290, 52);
        c.strokeStyle = 'rgba(246,191,94,.35)';
        c.strokeRect(LOGICAL_W - 307.5, 16.5, 289, 51);
        c.fillStyle = '#fff1ca';
        c.font = '18px Georgia';
        c.textAlign = 'right';
        c.fillText(this.level.title, LOGICAL_W - 32, 38);
        c.font = '13px Georgia';
        c.fillStyle = 'rgba(255,255,255,.72)';
        c.fillText('Relics ' + this.save.relics.length + '  •  F1 collision debug', LOGICAL_W - 32, 57);
        if (this.boss && this.boss.alive) {
            c.fillStyle = 'rgba(8,4,8,.72)';
            c.fillRect(230, 92, 500, 24);
            c.strokeStyle = '#f1b55a';
            c.strokeRect(230.5, 92.5, 499, 23);
            c.fillStyle = '#a62d2f';
            c.fillRect(235, 97, 490 * (this.boss.hp / this.boss.maxHp), 14);
            c.fillStyle = '#fff1ca';
            c.textAlign = 'center';
            c.font = '15px Georgia';
            c.fillText(this.boss.vulnerable ? 'Lantern Eater — vulnerable in Night' : 'Lantern Eater — blink to expose the light', 480, 86);
        }
        c.restore();
    }
    drawFloatingText(c, msg) {
        c.save();
        c.globalAlpha = 1 - Math.max(0, (msg.t - msg.max + .6) / .6);
        c.textAlign = 'center';
        c.font = '20px Georgia';
        c.fillStyle = 'rgba(9,5,13,.72)';
        c.fillRect(260, 460, 440, 42);
        c.strokeStyle = 'rgba(246,191,94,.35)';
        c.strokeRect(260.5, 460.5, 439, 41);
        c.fillStyle = '#fff1ca';
        c.fillText(msg.text, 480, 487);
        c.restore();
    }
    drawTitle(c) {
        this.camera.x = 0;
        ...
        c.fillStyle = '#ffe3a0';
        c.font = '64px Georgia';
        c.fillText('When the Dragon Blinks', LOGICAL_W / 2, 178);
        c.shadowBlur = 0;
        c.fillStyle = 'rgba(255,255,255,.82)';
        c.font = '20px Georgia';
        c.fillText('A mythic platformer inspired by Zhulong, the Torch Dragon', LOGICAL_W / 2, 216);
        c.fillStyle = 'rgba(10,5,13,.72)';
        c.fillRect(318, 304, 324, 168);
        c.strokeStyle = 'rgba(246,191,94,.42)';
        c.strokeRect(318.5, 304.5, 323, 167);
        const opts = ['Begin Journey', 'Level Select', 'Myth & History'];
        opts.forEach((o, i) => {
            const y = 360 + i * 44;
            const sel = i === this.titleSelection;
            c.fillStyle = sel ? '#ffd777' : '#fff1ca';
            c.font = sel ? '26px Georgia' : '22px Georgia';
            c.fillText((sel ? '◆ ' : '  ') + o + (sel ? ' ◆' : '  '), LOGICAL_W / 2, y);
        });
        c.fillStyle = 'rgba(255,255,255,.64)';
        c.font = '14px Georgia';
        c.fillText('Enter / A to select  •  H opens Myth & History', LOGICAL_W / 2, 506);
        c.restore();
    }
    drawLevelSelect(c) {
        this.drawBackground(c);
        ...
        c.fillStyle = '#ffe3a0';
        c.font = '48px Georgia';
        c.fillText('Choose a Shrine Path', 480, 96);
        levels.forEach((lvl, i) => {
            const x = 110 + i * 190, y = 190, unlocked = i <= this.save.highestUnlocked, sel = i === this.levelSelection;
            c.fillStyle = unlocked ? 'rgba(20,9,22,.76)' : 'rgba(20,20,24,.45)';
            c.fillRect(x, y, 160, 190);
            c.strokeStyle = sel ? '#ffd777' : 'rgba(246,191,94,.32)';
            c.lineWidth = sel ? 3 : 1;
            c.strokeRect(x + .5, y + .5, 159, 189);
            c.fillStyle = unlocked ? '#fff1ca' : '#777';
            c.font = '20px Georgia';
            c.fillText(lvl.title.replace('Level ', 'L'), x + 80, y + 42);
            c.font = '14px Georgia';
            wrapText(c, unlocked ? lvl.subtitle : 'Locked', x + 18, y + 78, 124, 20);
            c.fillStyle = unlocked ? '#ffd777' : '#555';
            c.font = '42px Georgia';
            c.fillText(unlocked ? (i === 3 ? '☲' : '◈') : '⌧', x + 80, y + 154);
        });
        c.fillStyle = 'rgba(255,255,255,.68)';
        c.font = '16px Georgia';
        c.fillText('←/→ choose  •  Enter start  •  Esc back', 480, 480);
        c.restore();
    }
    drawCodex(c) {
        this.drawBackground(c);
        c.save();
        c.fillStyle = 'rgba(8,4,12,.72)';
        c.fillRect(64, 48, 832, 444);
        c.strokeStyle = 'rgba(246,191,94,.38)';
        c.strokeRect(64.5, 48.5, 831, 443);
        c.fillStyle = '#ffe3a0';
        c.font = '42px Georgia';
        c.textAlign = 'center';
        c.fillText('Myth & History', 480, 92);
        c.font = '14px Georgia';
        c.fillStyle = 'rgba(255,255,255,.68)';
        c.fillText('This game is inspired by traditional accounts of Zhulong. Mythological details vary across sources and retellings.', 480, 116);
        const leftX = 94, top = 144;
        codexEntries.forEach((e, i) => {
            const unlocked = this.save.codex.includes(e.id);
            const sel = i === this.codexSelection;
            c.fillStyle = sel ? 'rgba(246,191,94,.16)' : 'transparent';
            c.fillRect(leftX - 10, top + i * 38 - 24, 300, 32);
            c.fillStyle = unlocked ? (sel ? '#ffd777' : '#fff1ca') : '#777';
            c.font = '18px Georgia';
            c.textAlign = 'left';
            c.fillText((unlocked ? '◇ ' : '◇ ') + e.title, leftX, top + i * 38);
        });
        const entry = codexEntries[this.codexSelection];
        const unlocked = this.save.codex.includes(entry.id);
        c.fillStyle = 'rgba(255,255,255,.06)';
        c.fillRect(430, 142, 410, 280);
        c.strokeStyle = 'rgba(246,191,94,.24)';
        c.strokeRect(430.5, 142.5, 409, 279);
        c.fillStyle = unlocked ? '#ffe3a0' : '#999';
        c.font = '28px Georgia';
        c.textAlign = 'left';
        c.fillText(entry.title, 458, 188);
        c.fillStyle = unlocked ? '#fff1ca' : '#aaa';
        c.font = '19px Georgia';
        wrapText(c, unlocked ? entry.body : 'Locked. ' + entry.unlockHint + '.', 458, 226, 350, 28);
        c.fillStyle = 'rgba(255,255,255,.64)';
        c.font = '15px Georgia';
        c.textAlign = 'center';
        c.fillText('↑/↓ choose  •  Esc back', 480, 466);
        c.restore();
    }
    drawLore(c) {
        if (!this.lorePanel)
            return;
        c.save();
        c.fillStyle = 'rgba(4,2,7,.66)';
        c.fillRect(0, 0, LOGICAL_W, LOGICAL_H);
        const x = 155, y = 82, w = 650, h = 376;
        c.fillStyle = 'rgba(18,8,20,.92)';
        c.fillRect(x, y, w, h);
        c.strokeStyle = 'rgba(246,191,94,.54)';
        c.lineWidth = 2;
        c.strokeRect(x + .5, y + .5, w - 1, h - 1);
        c.textAlign = 'center';
        c.fillStyle = '#ffe3a0';
        c.font = '34px Georgia';
        c.fillText(this.lorePanel.title, LOGICAL_W / 2, y + 52);
        let yy = y + 98;
        c.textAlign = 'left';
        for (const sec of this.lorePanel.sections) {
            ...
            c.font = '18px Georgia';
            c.fillText(sec.label, x + 42, yy);
            c.fillStyle = '#fff1ca';
            c.font = '20px Georgia';
            yy = wrapText(c, sec.text, x + 42, yy + 28, w - 84, 27) + 22;
        }
        c.textAlign = 'center';
        c.fillStyle = 'rgba(255,255,255,.62)';
        c.font = '16px Georgia';
        c.fillText('Enter / A / tap to continue', LOGICAL_W / 2, y + h - 28);
        c.restore();
    }
    drawPause(c) {
        c.save();
        ...
        c.fillRect(0, 0, LOGICAL_W, LOGICAL_H);
        c.fillStyle = 'rgba(18,8,20,.92)';
        c.fillRect(330, 170, 300, 180);
        c.strokeStyle = 'rgba(246,191,94,.5)';
        c.strokeRect(330.5, 170.5, 299, 179);
        c.textAlign = 'center';
        c.fillStyle = '#ffe3a0';
        c.font = '42px Georgia';
        c.fillText('Paused', 480, 230);
        c.fillStyle = '#fff1ca';
        c.font = '18px Georgia';
        c.fillText('Enter or Esc to resume', 480, 282);
        c.fillText('Day/Night: E/C/Y  •  Attack: J/X', 480, 316);
        c.restore();
    }
    drawLevelComplete(c) {
        c.save();
        ...
        c.fillStyle = '#ffe3a0';
        c.font = '48px Georgia';
        c.fillText('Shrine Path Restored', 480, 205);
        c.fillStyle = '#fff1ca';
        c.font = '22px Georgia';
        c.fillText('A new Myth & History entry has been unlocked.', 480, 255);
        c.font = '18px Georgia';
        c.fillText('Enter / tap for the next level  •  Esc for title', 480, 315);
        c.restore();
    }
    drawGameComplete(c) {
        c.save();
        c.fillStyle = 'rgba(4,2,7,.72)';
        c.fillRect(0, 0, LOGICAL_W, LOGICAL_H);
        c.textAlign = 'center';
        c.fillStyle = '#ffe3a0';
        c.font = '50px Georgia';
        c.fillText('The Dragon Blinks Again', 480, 110);
        c.fillStyle = 'rgba(18,8,20,.86)';
        c.fillRect(170, 150, 620, 260);
        c.strokeStyle = 'rgba(246,191,94,.45)';
        c.strokeRect(170.5, 150.5, 619, 259);
        c.textAlign = 'left';
        let yy = 195;
        c.font = '19px Georgia';
        ...
        c.fillStyle = '#fff1ca';
        c.font = '18px Georgia';
        c.fillText('Enter / tap: Myth & History  •  Esc: Title', 480, 470);
        c.restore();
    }
    drawDebug(c) {
        c.save();
        ...
        c.strokeRect(p.x - this.camera.x, p.y - this.camera.y, p.w, p.h);
        if (this.player.attackTimer > 0) {
            const a = this.player.attackRect();
            c.strokeStyle = '#ff0';
            c.strokeRect(a.x - this.camera.x, a.y - this.camera.y, a.w, a.h);
        }
        for (const s of this.solidsForRect({ x: this.camera.x, y: this.camera.y, w: LOGICAL_W, h: LOGICAL_H }))
            c.strokeRect(s.x - this.camera.x, s.y - this.camera.y, s.w, s.h);
        c.restore();
    }
}
...
function drawLabelled(c, label, text, x, y, w) {
    c.fillStyle = label === 'Myth' ? '#ffd777' : label === 'History' ? '#a9d6ff' : '#ffc0a0';
    c.font = '18px Georgia';
    c.fillText(label, x, y);
    c.fillStyle = '#fff1ca';
    c.font = '18px Georgia';
    return wrapText(c, text, x, y + 26, w, 24);
}
