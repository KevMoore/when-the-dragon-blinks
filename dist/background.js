...
function skyCanvas(game, th, qday, qcx, cy, storm) {
    ...
    if (!c2) {
        _skyKey = '';
        return null;
    } // iOS memory pressure — caller paints direct this frame
    _skyBufs[flip] = _sky;
    _skyCur = flip;
    _skyKey = key;
...
function paintSky(c, game, th, day, qcx, cy, storm) {
    c.fillStyle = '#08060d';
    ...
    c.globalAlpha = day;
    c.fillStyle = th.sun;
    c.shadowColor = mixHex(th.sun, '#ff8b3a', 0.4);
    c.shadowBlur = 46;
    c.beginPath();
...
export function drawSky(game, c) {
    ...
    if (cached)
        c.drawImage(cached, shx, shy);
    else {
        c.save();
        c.translate(shx, shy);
        paintSky(c, game, th, day, qcx, cy, storm);
        c.restore();
    } // cache unavailable — paint direct
...
function drawStorm(game, c) {
    ...
    if (lt > 0) {
        ...
        c.globalAlpha = a * 0.4;
        c.fillStyle = '#bcd2ff';
        c.fillRect(0, 0, LOGICAL_W, 310);
        c.globalAlpha = 1;
        c.globalCompositeOperation = 'source-over';
        c.globalAlpha = a;
        c.strokeStyle = '#eef4ff';
        c.shadowColor = '#9fc0ff';
        c.shadowBlur = 18;
        c.lineWidth = 2.4;
        c.lineCap = 'round';
        c.lineJoin = 'round';
...
function drawCloud(c, x, y, s, col, t = 0) {
    const b = (k) => 1 + Math.sin(t + k) * 0.1; // per-puff billow
    c.fillStyle = col;
    c.beginPath();
    c.arc(x, y, 11 * s * b(0), 0, Math.PI * 2);
    c.arc(x + 15 * s, y - 4 * s + Math.sin(t) * 2, 14 * s * b(1.7), 0, Math.PI * 2);
    c.arc(x + 32 * s, y, 11 * s * b(3.4), 0, Math.PI * 2);
    c.arc(x + 17 * s, y + 6 * s + Math.sin(t + 1) * 1.5, 13 * s * b(5.1), 0, Math.PI * 2);
    c.fill();
    c.strokeStyle = col;
    c.lineWidth = 3.5 * s;
    c.lineCap = 'round';
...
function drawClouds(game, c, day, storm = false) {
...
function drawPropImg(c, name, cx, baseY, targetH, alpha, tint, tintAmt = 0) {
    ...
    c.globalAlpha = alpha;
    c.drawImage(src, cx - w / 2, baseY - targetH, w, targetH);
    c.globalAlpha = 1;
}
...
function bakeThreeSlice(c, img, x, y, w, h, capFrac, mid) {
    ...
    for (let dx2 = x + capW - feather; dx2 < x + w - capW + feather; dx2 += dstMw, flip = !flip) {
        ...
        if (!flip)
            c.drawImage(img, m0, 0, srcW, ih, dx2, y, dw2, h);
        else {
    ...
    const cap = (srcX, dstX, fadeLeft) => {
        const t = document.createElement('canvas');
        t.width = Math.max(1, Math.round(capW));
        t.height = Math.max(1, Math.round(h));
        const tc = t.getContext('2d');
        if (!tc) {
            c.drawImage(img, srcX, 0, srcCap, ih, dstX, y, capW, h);
            return;
        }
        ...
        g.addColorStop(fadeLeft ? 0 : 1, 'rgba(0,0,0,1)');
        g.addColorStop(fadeLeft ? 1 : 0, 'rgba(0,0,0,0)');
...
export function drawThreeSlice(c, img, x, y, w, h, capFrac = 0.26, mid = [0.4, 0.6], name = '') {
    if (!name) {
        bakeThreeSlice(c, img, x, y, w, h, capFrac, mid);
        return;
    }
    ...
    if (!cvs) {
        ...
        if (!tc) {
            bakeThreeSlice(c, img, x, y, w, h, capFrac, mid);
            return;
        } // no ctx this frame — draw direct
        bakeThreeSlice(tc, img, 0, 0, cw, ch, capFrac, mid);
        if (!bakeLanded(tc, cw, ch)) {
            bakeThreeSlice(c, img, x, y, w, h, capFrac, mid);
            return;
        } // empty bake — don't cache
        if (_sliceCache.size > 300)
            _sliceCache.clear(); // safety valve
...
function tintedSprite(img, name, tint, tintAmt) {
    ...
    if (!tc)
        return cvs; // blank this frame, retried next (not cached)
    tc.drawImage(img, 0, 0);
    tc.globalCompositeOperation = 'source-atop';
    tc.globalAlpha = tintAmt;
    tc.fillStyle = tint;
    tc.fillRect(0, 0, cvs.width, cvs.height);
    if (!bakeLanded(tc, cvs.width, cvs.height))
        return cvs; // empty bake (iOS memory pressure) — don't cache it
    if (_tintCache.size > 220)
        _tintCache.clear(); // safety valve
    _tintCache.set(key, cvs);
    return cvs;
}
// draw an arbitrary image tinted toward a colour (atmospheric), at dw×dh
function drawTintedStill(c, img, dx, dy, dw, dh, tint, tintAmt, alpha, name = 'img') {
    c.globalAlpha = alpha;
    c.drawImage(tintedSprite(img, name, tint, tintAmt), dx, dy, dw, dh);
    c.globalAlpha = 1;
}
...
export function glowSprite(color) {
    ...
    if (!gc)
        return g; // blank this frame, retried next (not cached)
...
export function drawGlow(c, x, y, r, color, alpha) {
    c.save();
    c.globalCompositeOperation = 'lighter';
    c.globalAlpha = alpha;
...
function bakeLanded(tc, w, h) {
    try {
        ...
        return true;
    } // unreadable (shouldn't happen with local assets) — assume fine
}
...
function terrainPattern(c, themeName, th, img) {
    ...
    if (!tc)
        return c.createPattern(img, 'repeat'); // uncached fallback, retries next frame
    tc.drawImage(img, 0, 0);
    tc.globalCompositeOperation = 'source-atop';
    tc.globalAlpha = 0.42;
    tc.fillStyle = mixHex(th.soilTop, th.soilBot, 0.45);
    tc.fillRect(0, 0, tile.width, tile.height);
    if (!bakeLanded(tc, tile.width, tile.height))
...
export function drawParallax(game, c) {
    ...
    if (mtn?.ready) {
        ...
        for (let x = start; x < LOGICAL_W + W; x += W)
            drawTintedStill(c, img, x, y0, W, H, mixHex(ridgeQ, hazeQ, 0.72), 0.55, 1, 'mtn');
    }
    ...
    for (let layer = 0; layer < N; layer++) {
        ...
        const pts = [];
        let crest = LOGICAL_H;
        for (let x = -20; x <= LOGICAL_W + 20; x += 10) {
        ...
        c.globalAlpha = 1;
        c.fillStyle = grad; // opaque → clean occlusion between planes
        c.beginPath();
        ...
        c.globalAlpha = 0.10 + far * 0.16;
        const hz = c.createLinearGradient(0, crest - 4, 0, crest + 46);
        hz.addColorStop(0, mixHex(haze, skyLight, far * 0.4));
        hz.addColorStop(1, 'rgba(0,0,0,0)');
        c.fillStyle = hz;
        c.beginPath();
        c.moveTo(-20, crest + 60);
        for (const p of pts)
            c.lineTo(p.x, p.y);
        c.lineTo(LOGICAL_W + 20, crest + 60);
        c.closePath();
        c.fill();
        c.restore();
        // bright rim-light along every crest so each silhouette pops
        c.globalAlpha = 0.5 + far * 0.35;
        c.strokeStyle = mixHex(base, skyLight, 0.6);
        c.lineWidth = 1.5;
        c.beginPath();
...
export function drawForeground(game, c) {
    ...
    for (let i = first; i <= last; i++) {
        if (hash(i * 17) > 0.5)
            continue; // big gaps between clumps
    ...
    for (let y = y0; y <= y1; y++)
        for (let x = x0; x <= x1; x++) {
            ...
            if (ch === 'o') {
                ...
                if (game.tileAt(x - 1, y) === 'o' && x > x0)
                    continue;
                let s = x;
                while (game.tileAt(s - 1, y) === 'o')
                    s--;
                let run = 1;
                while (game.tileAt(s + run, y) === 'o')
                    run++;
                ...
                if (spr?.ready) {
                    ...
                    c.shadowBlur = 8;
                    drawThreeSlice(c, spr.img, rx - 6, sy - 2, runW + 12, 44, 0.15, [0.3, 0.7], 'platform');
                    c.restore();
                }
                else {
                    c.fillStyle = mixHex(tt.soilTop, tt.soilBot, 0.4);
                    c.fillRect(rx, sy + 4, runW, 8);
                    c.fillStyle = tt.grass;
                    c.fillRect(rx, sy, runW, 5);
                }
            }
            else if (ch === 'D' || ch === 'N') {
                ...
                if (game.tileAt(x - 1, y) === ch && x > x0)
                    continue;
                let s = x;
                while (game.tileAt(s - 1, y) === ch)
                    s--;
                let run = 1;
                while (game.tileAt(s + run, y) === ch)
                    run++;
...
function drawSpan(game, c, th, a, b, camX, camY, bottom, topY) {
    ...
    if (topImgRaw && propReady['terrain_top']) {
        ...
        for (let sx0 = wx0; sx0 < rightX; sx0 += step) {
            ...
            if (Math.abs(y1 - y0) > w2 * 2.05)
                continue; // steeper than 2:1 = cliff, bare rock (2-tile steps stay turfed + climbable)
            ...
            if (su + srcW <= TW)
                c.drawImage(topImg, su, 0, srcW, TH, dx0, dy0, w2 + 1.5, dstH);
...
function drawStatePlatform(game, c, x, y, w, state) {
...
export function drawWind(game, c) {
    ...
    for (const z of game.level.windZones || []) {
        ...
        if (sx > LOGICAL_W || sx + z.w < 0)
            continue;
        c.save();
        c.beginPath();
        c.rect(sx, sy, z.w, z.h);
        c.clip(); // keep the effect inside the shaft
        ...
        wash.addColorStop(1, `rgba(${col},${game.world === 'day' ? 0.05 : 0.09})`);
        c.fillStyle = wash;
        c.fillRect(sx, sy, z.w, z.h);
        ...
        for (let i = 0; i < N; i++) {
            ...
            c.lineWidth = 1.6;
            c.beginPath();
            c.moveTo(lane, yv + len);
            c.lineTo(lane + Math.sin(game.time + i) * 3, yv);
            c.stroke();
        }
        c.restore();
    }
...
export function drawVignette(c) {
    if (!_vignette) { // rendered once, blitted forever
        ...
        if (!vc) {
            _vignette = null;
            return;
        } // no vignette this frame, retry next
