...
export class Game {
    ...
    constructor(ctx) {
        ...
        this.lightningT = 0;
        this.lightningX = 0;
        this.lightningY = 300; // boss-arena storm + lightning
        this.flash = 0;
        ...
        this.comboT = 0;
        this.scorePops = [];
        this.dragonMeter = 0; // 0..1 — fills from Torch Embers + gems; full → become Zhulong
        this.nova = 0.35; // 0..1 inner energy — full → hold fire to unleash a Nova burst
        this.novaT = 0;
        this.novaX = 0;
        ...
        this._pausedUi = false; // tracks the body.paused class + ❚❚/▶ glyph
        ...
        if (af && this.save.settings.autoFire) {
            af.classList.add('on');
            af.setAttribute('aria-pressed', 'true');
        }
    }
    ...
    startLevel(i, withIntro = true) {
        ...
        this.dragonMeter = 0;
        this.embers = [];
        this.player.dragonTime = 0;
        this.player.dragonTrail = [];
        this.transformT = 0;
        this.nova = 0.35;
        this.novaT = 0;
        this.deathT = 0;
    ...
    snapEnemySpawn(en) {
        ...
        if (walker) {
            ...
            let guard = 0;
            while (this.overlapsSolid(en.rect()) && en.y > TILE && guard++ < 30)
                en.y -= TILE;
            en.baseY = en.y;
        }
    }
    ...
    spawnEmbers(x, y, n) {
        for (let i = 0; i < n; i++)
            this.embers.push({ x, y, vx: rand(-70, 70), vy: rand(-190, -90), life: 9 });
        if (this.embers.length > 120)
            this.embers.splice(0, this.embers.length - 120);
    }
    updateEmbers(dt) {
    ...
    drawGems(c) {
        for (const g of this.gems) {
            ...
            c.translate(x, y);
            c.rotate(Math.PI / 4);
            c.shadowColor = '#ffcf6a';
            c.shadowBlur = 12;
            const grad = c.createLinearGradient(-8, -8, 8, 8);
            grad.addColorStop(0, '#fff1c0');
            grad.addColorStop(0.5, '#ffb43c');
            grad.addColorStop(1, '#d9541f');
            c.fillStyle = grad;
            c.fillRect(-8, -8, 16, 16);
            c.strokeStyle = 'rgba(255,240,200,.85)';
            c.lineWidth = 1.5;
    ...
    updateTransform(dt) {
        ...
        if (Math.abs(p - 0.5) < 0.03) {
            ...
            this.particles.ring(cx, cy, 40, 420, '#ffd777');
            this.particles.sparks(cx, cy, 46, '#ff9d4d');
            this.audio.sfx('boss');
        }
    ...
    startDragon() {
        this.transformT = 0;
        this.dragonMeter = 0;
        this.player.dragonTime = 12;
        this.player.dragonTrail = [];
        this.player.dragonFireCd = 0;
    ...
    completeLevel() {
        ...
        this.clearing = true;
        this.clearOutro = this.level.outroLore;
    ...
    updateBossDeath(dt) {
        ...
        this.bossDeathT -= dt;
        if (p > 0.16)
            this.dayAmount = clamp(this.dayAmount + dt * 0.7, this.dayAmount, 1); // dawn returns
        ...
        if (p > 0.16 && p < 0.62) { // eruption build
            ...
            if (Math.random() < 0.1 + p * 0.22) {
                this.camera.addTrauma(0.26 + p * 0.4);
                this.particles.ring(this.bossDeathX, this.bossDeathY, 24, 320, '#ffd777');
            }
        }
        if (!this.bossClimax && p > 0.62) { // the shatter
            this.bossClimax = true;
            this.flash = 1;
            this.flashColor = '#fff6dc';
            this.camera.addTrauma(1.25);
            this.audio.sfx('boss');
        ...
        if (p > 0.72)
            this.particles.embers(this.bossDeathX, this.bossDeathY - 24, 2); // rising to the dragon
    ...
    drawBossDeathCinematic(c) {
        ...
        if (p < 0.2) {
            ...
            v.addColorStop(1, `rgba(2,1,4,${0.85 * t})`);
            c.fillStyle = v;
            ...
            c.shadowBlur = 16;
            c.beginPath();
            c.moveTo(x, y - 20 - 50 * (1 - t));
            c.lineTo(x + 4, y);
            c.lineTo(x - 3, y + 34);
            c.lineTo(x + 2, y + 40 + 50 * (1 - t));
            c.stroke();
            c.restore();
            c.globalAlpha = 1;
            c.shadowBlur = 0;
        ...
        if (p > 0.12) {
            ...
            core.addColorStop(0, `rgba(255,250,225,${0.6 + 0.4 * rp})`);
            core.addColorStop(0.45, `rgba(255,185,95,${0.35 * rp})`);
            ...
            for (let r = 0; r < 3; r++) {
                ...
                if (rr > 0 && rr < 1) {
                    c.globalAlpha = (1 - rr) * 0.6;
                    c.strokeStyle = '#ffe6a0';
                    c.lineWidth = 5 * (1 - rr) + 1;
                    c.beginPath();
                    c.arc(x, y, rr * 560, 0, Math.PI * 2);
                    c.stroke();
                }
            }
            c.globalAlpha = 1;
        ...
        if (p > 0.6 && p < 0.9)
            this.drawGlassFracture(c, x, y, clamp((p - 0.6) / 0.28, 0, 1));
        // Phase 4 — Zhulong rises over the freed dawn
        if (p > 0.68)
            this.drawRisingDragon(c, x, y, clamp((p - 0.68) / 0.32, 0, 1));
    }
    drawGlassFracture(c, x, y, fp) {
        c.save();
        c.globalAlpha = (1 - fp) * 0.5;
        c.fillStyle = '#fff6e2';
        c.fillRect(0, 0, LOGICAL_W, LOGICAL_H);
        c.globalAlpha = 0.85 * (1 - fp * 0.5);
        c.strokeStyle = '#ffffff';
        c.shadowColor = '#ffe6a0';
        c.shadowBlur = 8;
        c.lineWidth = 1.6;
        c.lineCap = 'round';
        ...
        for (let i = 0; i < N; i++) {
            ...
            for (let seg = 0; seg < 5 && len < maxLen; seg++) {
                ...
                c.lineTo(px, py);
                if (seg > 0 && seg % 2 === 0) {
                    const ba = ang + (Math.sin(i + seg) > 0 ? 0.6 : -0.6);
                    c.moveTo(px, py);
                    c.lineTo(px + Math.cos(ba) * 42, py + Math.sin(ba) * 42);
                    c.moveTo(px, py);
                }
            }
            c.stroke();
    ...
    drawRisingDragon(c, x, y, dp) {
        ...
        c.fillStyle = halo;
        c.beginPath();
        c.arc(x, cy - 40, 320 * dp + 90, 0, Math.PI * 2);
        c.fill();
        ...
        c.shadowBlur = 22 * dp;
        c.beginPath();
        c.ellipse(hx, hy, 16 * dp + 4, 10 * dp + 3, 0, 0, Math.PI * 2);
        c.fill();
        c.shadowBlur = 0;
        c.fillStyle = '#ffe08a';
        c.beginPath();
        c.arc(hx + 4 * dp, hy - 2, 2.5 * dp + 1, 0, Math.PI * 2);
        c.fill();
        c.restore();
        c.globalAlpha = 1;
        c.shadowBlur = 0;
            ...
            for (const s of this.solidsForRect(rect)) {
                ...
                if (e.grounded && sx !== 0) {
                    ...
                    for (let up = 5; up <= maxUp; up += 5) {
                        if (!this.overlapsSolid({ x: e.x, y: e.y - up, w: e.w, h: e.h })) {
                            e.y -= up;
                            climbed = true;
                            break;
                        }
    ...
    update(dt) {
        ...
        if (this.level.isBoss) {
            ...
            if (this.stormT <= 0) {
                ...
                this.lightningY = 300;
                this.flash = Math.max(this.flash, rage ? 0.34 : 0.5);
                this.flashColor = '#dbe8ff';
                this.camera.addTrauma(rage ? 0.2 : 0.35);
                if (!rage || Math.random() < 0.55)
                    this.audio.sfx('boss');
            }
        }
    ...
    updatePlaying(dt) {
        ...
        if (this.input.just('autofire')) {
            ...
            if (b) {
                b.classList.toggle('on', s.autoFire);
                b.setAttribute('aria-pressed', String(s.autoFire));
            }
        ...
        // lantern-hearts: magnetic pickup that rekindles 1 HP (or +100 if full)
        {
            ...
            for (const ht of this.hearts) {
                ...
                if (overlap(p.rect(), { x: ht.x, y: ht.y, w: 18, h: 18 })) {
                    ...
                    if (p.hp < p.maxHp) {
                        p.hp++;
                        this.flashText('A lantern-heart rekindles you.');
                    }
                    else
                        this.addScore(100, ht.x, ht.y);
                    this.audio.sfx('checkpoint');
                    this.particles.sparks(ht.x + 9, ht.y + 9, 14, '#ff9d6a');
                    this.particles.ring(ht.x + 9, ht.y + 9, 8, 150, '#ffb24a');
                }
            }
    ...
    updatePause() {
        ...
        if (this.input.pointer?.clicked) { // tap an option (mobile-navigable)
            ...
            for (let i = 0; i < n; i++)
                if (y > 250 + i * 34 - 24 && y < 250 + i * 34 + 10) {
    ...
    updateLevelComplete() {
        if (this.input.just('confirm') || this.input.pointer?.clicked) {
            ...
            if (!this.level.hidden && this.guqinDueFor(this.currentLevelIndex)) {
                this.startGuqin(next);
                return;
            }
            if (!this.level.hidden && this.dawnDueFor(this.currentLevelIndex)) {
                this.startDawn(next);
                return;
            }
    ...
    updateGameComplete() {
        ...
        if (this.input.pointer?.clicked) { // tap an option directly
            ...
            if (y > 418 && y < 454)
                for (let i = 0; i < n; i++)
                    if (Math.abs(x - (LOGICAL_W / 2 - 220 + i * 220)) < 104) {
                        this.completeSelection = i;
                        this.chooseComplete();
                        return;
                    }
        }
    ...
    render() {
        ...
        this.camera.x -= shx;
        this.camera.y -= shy; // undo the render-only shake offset
        c.restore();
        if (this.showFps) {
            const now = performance.now();
            this._fpsMs = this._fpsMs * 0.95 + (now - t0) * 0.05; // render cost EMA
            const frame = this._fpsLast ? now - this._fpsLast : 16.7;
            this._fpsLast = now;
            this._frameMs = (this._frameMs ?? 16.7) * 0.95 + frame * 0.05;
            c.save();
            c.fillStyle = 'rgba(0,0,0,.6)';
            c.fillRect(6, LOGICAL_H - 26, 210, 20);
            c.fillStyle = '#8fd9a8';
            c.font = '12px monospace';
            c.textAlign = 'left';
            c.fillText(`render ${this._fpsMs.toFixed(1)}ms · ${(1000 / this._frameMs).toFixed(0)}fps`, 12, LOGICAL_H - 12);
            c.restore();
        }
    }
    drawWorld(c) {
        ...
        if (this.player.dragonTime > 0) {
            c.save();
            c.globalCompositeOperation = 'lighter';
            c.globalAlpha = 0.06 + 0.03 * Math.sin(this.time * 6);
            c.fillStyle = '#ffb84a';
            c.fillRect(0, 0, LOGICAL_W, LOGICAL_H);
            c.restore();
            c.globalAlpha = 1;
        ...
        for (const pl of this.platforms) {
            ...
            if (sprite?.ready) {
                ...
                if (pl.crumble) {
                    ...
                    c.lineWidth = 2;
                    c.beginPath();
                    c.moveTo(x + pl.w * 0.42, y - 2);
                    c.lineTo(x + pl.w * 0.52, y + pl.h + 10);
                    c.stroke();
                }
            }
            else {
    ...
    drawObjects(c) {
        ...
        if (!this.level.isBoss) {
            ...
            if (st) {
                ...
                g.addColorStop(0, `rgba(${glow},.5)`);
                g.addColorStop(0.5, `rgba(${glow},.18)`);
                g.addColorStop(1, 'rgba(0,0,0,0)');
                c.fillStyle = g;
                c.beginPath();
                c.arc(cx, midY, 122, 0, Math.PI * 2);
                c.fill();
                c.globalAlpha = pulse * 0.5;
                c.strokeStyle = `rgba(${glow},.7)`;
                c.lineWidth = 2; // shimmer ring
                c.beginPath();
                c.arc(cx, midY, 46 + 6 * Math.sin(this.time * 2), 0, Math.PI * 2);
                c.stroke();
                for (let k = 0; k < 6; k++) { // rising light motes
                    ...
                    c.globalAlpha = (1 - ph) * pulse * 0.9;
                    c.fillStyle = `rgba(${glow},1)`;
                    c.beginPath();
                    c.arc(mx, my, 2.2 * (1 - ph) + 0.6, 0, Math.PI * 2);
                    c.fill();
                }
                c.restore();
                c.globalAlpha = 1;
                c.save();
                c.shadowColor = `rgba(${glow},1)`;
                c.shadowBlur = 22 + Math.sin(this.time * 3) * 8;
                st.draw(c, cx, by, h);
                c.restore();
            }
            else {
    ...
    drawTransformCinematic(c) {
        ...
        vg.addColorStop(0, 'rgba(8,3,2,0)');
        vg.addColorStop(1, `rgba(6,2,2,${dark})`);
        c.fillStyle = vg;
        c.fillRect(0, 0, LOGICAL_W, LOGICAL_H);
        // rotating god-rays from the hero
        c.save();
        c.globalCompositeOperation = 'lighter';
        c.globalAlpha = swell * 0.5;
        c.translate(cx, cy);
        c.rotate(this.time * 1.1);
        for (let i = 0; i < 12; i++) {
            c.rotate(Math.PI * 2 / 12);
            c.fillStyle = 'rgba(255,200,90,.5)';
            c.beginPath();
            c.moveTo(0, 0);
            c.lineTo(580, -26);
            c.lineTo(580, 26);
        ...
        c.globalCompositeOperation = 'lighter';
        for (let k = 0; k < 3; k++) {
            const rp = p * 1.4 - k * 0.18;
            if (rp > 0 && rp < 1) {
                c.globalAlpha = (1 - rp) * 0.6;
                c.strokeStyle = '#ffd777';
                c.lineWidth = 4;
                c.beginPath();
                c.arc(cx, cy, rp * 460, 0, Math.PI * 2);
                c.stroke();
            }
        }
        c.restore();
        ...
        c.save();
        c.globalAlpha = clamp(p / 0.3, 0, 1) * clamp((1 - p) / 0.25, 0, 1);
        c.shadowColor = '#ff4a28';
        c.shadowBlur = 40;
        c.fillStyle = '#160406';
        c.beginPath();
        c.ellipse(ex, ey, 150, 60 * (0.12 + open * 0.88), 0, 0, Math.PI * 2);
        c.fill();
        ...
        c.fillStyle = iris;
        c.beginPath();
        c.ellipse(ex, ey, 26, 58 * open, 0, 0, Math.PI * 2);
        c.fill();
        c.restore();
        c.globalAlpha = 1;
        c.shadowBlur = 0;
        ...
        if (tin > 0) {
            ...
            c.globalAlpha = tin;
            c.textAlign = 'center';
            c.shadowColor = '#ff6a2a';
            c.shadowBlur = 34;
            c.fillStyle = '#ffdf9a';
            c.font = 'bold 88px Georgia';
    ...
    drawMangaClear(c) {
        ...
        c.fillStyle = `rgba(252,246,232,${Math.max(0, 0.92 - p * 5)})`;
        c.fillRect(0, 0, LOGICAL_W, LOGICAL_H);
        c.globalAlpha = clamp(p * 5, 0, 1) * 0.45;
        c.fillStyle = '#140a10';
        c.fillRect(0, 0, LOGICAL_W, LOGICAL_H);
        c.globalAlpha = 1;
        // radial action lines
        ...
        for (let i = 0; i < n; i++) {
            ...
            c.lineTo(Math.cos(a) * 900, Math.sin(a) * 900);
            c.stroke();
        }
        c.restore();
        // screentone dots in the corners
        c.save();
        c.fillStyle = 'rgba(20,10,14,.25)';
        for (let gx = 0; gx < LOGICAL_W; gx += 12)
            for (let gy = 0; gy < LOGICAL_H; gy += 12) {
                const edge = Math.min(gx, LOGICAL_W - gx, gy, LOGICAL_H - gy);
                if (edge < 90) {
                    c.beginPath();
                    c.arc(gx, gy, 2.2 * (1 - edge / 90), 0, Math.PI * 2);
                    c.fill();
                }
            }
        c.restore();
        ...
        c.save();
        c.translate(cx + slide, cy - 18);
        c.rotate(-0.11);
        c.fillStyle = '#c8302a';
        c.fillRect(-370, -54, 740, 108);
        c.fillStyle = '#1a0a10';
        c.fillRect(-370, -54, 740, 8);
        c.fillRect(-370, 46, 740, 8);
        c.textAlign = 'center';
        c.fillStyle = '#ffe0a0';
        c.font = 'bold 60px Georgia';
        ...
        if (stamp > 0) {
            c.save();
            c.globalAlpha = Math.min(1, stamp);
            const sc = 1.5 - 0.5 * Math.min(1, stamp);
            c.translate(cx, cy + 128);
            c.scale(sc, sc);
            c.rotate(0.05);
            c.fillStyle = '#ffd777';
            c.strokeStyle = '#1a0a10';
            c.lineWidth = 5;
            c.font = 'bold 74px Georgia';
            c.textAlign = 'center';
            c.strokeText('過關', 0, 0);
            c.fillText('過關', 0, 0);
            c.restore();
    ...
    drawHearts(c) {
        for (const ht of this.hearts) {
            ...
            c.translate(x, y);
            c.fillStyle = '#d94a3a';
            c.shadowColor = '#ff8b5a';
            c.shadowBlur = 10;
            c.beginPath();
            c.ellipse(0, 0, 7.5, 9, 0, 0, Math.PI * 2);
            c.fill();
            c.shadowBlur = 0;
            c.fillStyle = '#3a1410';
            c.fillRect(-4, -11, 8, 3);
    ...
    drawEmbers(c) {
        ...
        for (const e of this.embers) {
            ...
            c.shadowColor = '#ff9d4d';
            c.shadowBlur = 10;
            c.fillStyle = '#ffd777';
            c.beginPath();
            c.arc(x, y, 2.5 * pulse + 1.6, 0, Math.PI * 2);
            c.fill();
        }
        c.restore();
        c.globalAlpha = 1;
